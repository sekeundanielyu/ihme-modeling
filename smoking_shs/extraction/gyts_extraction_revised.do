...
// run search for variables (currently case sensative - be aware of this) 
	...
	lookat("finalwgt" "strat" "psu" "inr84" /// 
	...
	"where do you smoke" "In what kind of place do you live" "past 30 days") /// 				These are the variable names you want to search for
...
// open search file for example purposes
	use "/snfs3/WORK/05_risk/temp/explore/second_hand_smoke/shs_gyts_search_vars.dta" , clear  
	
// use "C:/Users/lalexan1/Desktop/shs_gyts_search_vars.dta" , clear 
...
save "/snfs1/WORK/05_risk/02_models/smoking_shs/01_exp/01_tabulate/data/raw/check_STP.dta", replace



// propogate to codebooks 
	populate_codebooks , ///
	mirror_root(/snfs3/WORK/05_risk/temp/explore/second_hand_smoke/GLOBAL_YOUTH_TOBACCO_SURVEY) //	
...
// run extraction
	...
	job_name(shs_gyts_extract_trial) ///																		This is what our final file will be called
...
// open extration file for example purposes
	use "/snfs3/WORK/05_risk/temp/explore/second_hand_smoke/shs_gyts_extract_trial.dta" , clear  
...
// A loop to clean out nulls created during extraction. Any variable that is null in a survey is recoded to -9999 during extraction
	...
	save "/snfs1/WORK/05_risk/risks/smoking_shs/data/exp/01_tabulate/raw/shs_gyts_extract.dta", replace
...
use "$prefix/WORK/05_risk/risks/smoking_shs/data/exp/01_tabulate/raw/shs_gyts_extract.dta", clear
...
// ************************ SMOKING INDICATOR ****************************************// 
	...
	keep path psu strata weight urban sex parent smoker
...
// Loop through countries, sexes and ages and calculate secondhand smoke prevalence among nonsmokers using survey weights 
		...
		foreach path of local paths {
			foreach sex of local sexes { 
				...
				count if path == "`path'" & sex == `sex' & parent != . 
	...
	// Get stored prevalence calculations from matrix
		...
		getmata path sex sample_size mean_hh_shs se_hh_shs 
...
// Loop through countries, sexes and ages and calculate secondhand smoke prevalence among nonsmokers using survey weights
		...
		foreach path of local paths {
			foreach urban of local urbanicities {
				...
				count if path == "`path'" & sex == `sex' & urban == `urban' & smoker != 1 & parent != . 
				local sample_size = r(N)
...
// Get stored prevalence calculations from matrix
		...
		getmata path sex urban sample_size mean_hh_shs se_hh_shs 
		drop if _n == 1 // drop top row which was a placeholder for the matrix created to store results
	...
	replace gyts_names = "" if gyts_names != "" & iso3 == "IND" & urban == . 
		...
		tempfile almost_all
		save `almost_all', replace
		
// Bring in country codes 
	clear
	#delim ;
	odbc load, exec("SELECT ihme_loc_id, location_name, location_id, location_type, super_region_name, region_name
...
// Bring in extracted GYTS data 

	use `almost_all', clear 
...
// Append to larger GYTS file 
	...
	// Set variables that are always tracked
		rename se_hh_shs standard_error
		rename mean_hh_shs mean
		gen survey = "GYTS"
	...
	save "$prefix/WORK/05_risk/risks/smoking_shs/data/exp/01_tabulate/prepped/gyts_subnationals.dta", replace
