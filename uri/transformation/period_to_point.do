...
// create directories and start log
	...
	local tmp_dir "$prefix/WORK/04_epi/01_database/02_data/${functional}/03_review/03_temp/${date}"
	cap mkdir "$prefix/WORK/04_epi/01_database/02_data/${functional}/03_review/03_temp"
	cap mkdir "`tmp_dir'"
	cap mkdir "`out_dir'/03_temp/${date}"
	cap mkdir "`out_dir'/04_diagnostics/${date}"
	cap mkdir "`out_dir'/05_outputs/${date}"
	capture log close
...
// save final, send this path to data analyst for upload
	outsheet using "`out_dir'/05_upload/${functional}_${date}.csv", comma replace
