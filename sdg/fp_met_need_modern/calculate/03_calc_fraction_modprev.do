...
foreach country of local countries {
	...
	foreach dta of local dtas {
		...
		forvalues i=15(5)45 { // loop through age groups from 15-50 by 5 years
		count if ihme_age_yr>=`i' & ihme_age_yr<(`i'+5) // make sure there are women in that age group before trying to calcuate prevalence
			...
			if r(N)!=0 {
				...
				** ever married women population (includes currently married and formerly married women)
				...
				// gen avg interview date
					...
					keep agegroup modall_prev modall_var modcurrmarr_prev modcurrmarr_var modevermarr_prev modevermarr_var  iso3 ihme_start_year ihme_end_year filename survey ihme_type ihme_file_name sample
							
					order iso3 survey ihme_start_year ihme_end_year agegroup filename  modall_prev modall_var modcurrmarr_prev modcurrmarr_var modevermarr_prev modevermarr_var sample
	...
	restore
				...
				} // closing loop on sum of women in that age group
				
			} // closing age group loop
...
use `master', clear
drop foo
save "J:\Project\Coverage\Contraceptives\2015 Contraceptive Prevalence Estimates\output\MASTER PREVALENCE FILES\master_modern_contra_${date}.dta", replace
