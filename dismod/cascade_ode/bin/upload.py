...
def upload_file(mvid, filepath, table, cols):
    print 'Uploading %s to mvid %s' % (filepath, mvid)
    load_data_query = """
        LOAD DATA INFILE '{fp}'
        INTO TABLE {table}
        FIELDS
...
def upload_model(mvid):
    ...
    for i, m in enumerate(prior_matches):
        print 'Prior file #:%s' % i
        upload_file(mvid, m,
            'epi.model_prior',
            [   'year_id','location_id','sex_id','age_group_id','age',
                'measure_id','mean','lower','upper'])

    print 'Uploading adjusted data'
    adj_data_file = '%s/%s/full/locations/1/outputs/both/2000/model_data_adj.csv' % (settings['cascade_ode_out_dir'], mvid)
    upload_file(mvid, adj_data_file,
        'epi.model_data_adj',
        ['input_data_key', 'sex_id', 'year_id', 'mean', 'lower', 'upper'])

    print 'Uploading effects'
    me_file = '%s/%s/full/locations/1/outputs/both/2000/model_effect.csv' % (settings['cascade_ode_out_dir'], mvid)

    upload_file(mvid, me_file,
        'epi.model_effect',
        ['measure_id','parameter_type_id','cascade_level_id','study_covariate_id','country_covariate_id','asdr_cause','location_id','mean_effect','lower_effect','upper_effect'])
...
if __name__=="__main__":
    ...
    matches = []
    prior_matches = []
    searchdir = '/ihme/epi/panda_cascade/dev/%s/locations' % mvid
    locdirs = glob('%s/*' % searchdir)
    for ld in locdirs:
        for sex in ['male', 'female','both']:
            for year in range(1990, 2016, 5):
    ...
    new_mvid = copy_model_version(mvid, description)
    copy_model_parameters(mvid, new_mvid)

    for i, m in enumerate(matches):
        print 'File #:%s' % i
        upload_file(new_mvid, m,
            'epi.model_estimate_fit',
            [   'location_id','year_id','sex_id','age_group_id','measure_id',
                'mean','lower','upper'])
    ...
    print 'Uploading adjusted data'
    adj_data_file = '/ihme/epi/panda_cascade/dev/%s/locations/1/outputs/both/2000/model_data_adj.csv' % mvid
    upload_file(new_mvid, adj_data_file,
        'epi.model_data_adj',
        ['input_data_key', 'sex_id', 'year_id', 'mean', 'lower', 'upper'])

    print 'Uploading effects'
    me_file = '/ihme/epi/panda_cascade/dev/%s/locations/1/outputs/both/2000/model_effect.csv' % mvid
    upload_file(new_mvid, me_file,
        'epi.model_effect',
        ['measure_id','parameter_type_id','cascade_level_id','study_covariate_id','country_covariate_id','asdr_cause','location_id','mean_effect','lower_effect','upper_effect'])
