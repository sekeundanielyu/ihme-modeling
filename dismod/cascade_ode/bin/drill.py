...
class Cascade(object):
    ...
    def __init__(
        ...
        if reimport:
            ...
            self.ccov_means = ii.ccov_means
            self.data = ii.data
            self.mtall = ii.mtall
            self.single_param = ii.single_param
            self.model_params = ii.model_params
            self.alldata = pd.concat([self.data, self.mtall])
        ...
        self.alldata[['atom','subreg','region','super']] = self.alldata[['atom','subreg','region','super']].replace({'none':'nan'})
        for c in ['atom','subreg','region','super']:
            self.alldata[c] = self.alldata[c].apply(lambda x: "{:.1f}".format(float(x)))
        self.alldata.ix[self.alldata.atom=='nan', 'atom'] = 'none'
        ...
        self.loctree = lt.construct(location_set_version_id)
...
class Cascade_loc(object):
    ...
    def __init__(self, loc, sex, year, cascade, parent_loc=None, reimport=True,
        ...
        """
        self.loc = "{:.1f}".format(loc)
        self.year = year
        self.loctree = cascade.loctree
        self.locnode = self.loctree.get_node_by_id(int(loc))
        self.locname = self.locnode.info['dismod_name']
        self.loclvl = self.locnode.info['dismod_level']
        self.loclvl_id = cascade_level_ids[self.loclvl]
        ...
        try: os.makedirs(self.in_dir)
        except: pass
        try: os.makedirs(self.out_dir)
        except: pass
        try: os.makedirs(self.draws_dir)
        except: pass
        try:
        ...
        self.data[['atom','subreg','region','super']] = self.data[['atom','subreg','region','super']].replace({'none':'nan'})
        for c in ['atom','subreg','region','super']:
            self.data[c] = self.data[c].apply(lambda x: "{:.1f}".format(float(x)))
        self.data.ix[self.data.atom=='nan', 'atom'] = 'none'
    ...
    def get_location_level(self, location_id):
        return self.loctree.get_node_by_id(location_id).info['dismod_level']
    ...
    def gen_data(self, location_id, sex_id, drop_emr=False):
        ...
        for lvl in cascade_levels:
            try: data[lvl] = data[lvl].replace('nan', 'none')
            except: pass
        ...
        if self.loclvl == 'world':
            ...
            if int(float(self.loc)) in parent_data.location_id.unique():
                ...
                parent_data = parent_data[
                    ...
                    (parent_data.x_sex == prior_sex) &
                    (parent_data.x_local == 0)]
            print 'appending parent predictions as data'
        ...
        ally_data = ally_data[ally_data.a_data_id.notnull()]
    ...
    def gen_predin(self):
        ...
        children_df = []
        if self.loclvl == 'world':
            years_to_predict = range(1990, 2016, 5)
        else:
        ...
        for year in years_to_predict:
            for child in locnode.children:
                ...
                child_loc = "{:.1f}".format(child.id)
                child_lvl = child.info['dismod_level']
                locs_with_data = list(
        ...
        if len(locnode.children) > 0:
            child_lvl = locnode.children[0].info['dismod_level']
    ...
    def center_series(self, series, central_value):
        """ Centers a series such that the mean is zero """
        pass
    ...
    def summarize_posterior(self, burn_pct=20):
        ...
        #assert self.dismod_finished, 'Dismod has not yet been run for' \
        #    ' this location'
        ...
        ccs['covariate_name_short'] = ccs.effect.apply(lambda x: "_".join(x.split("_")[4:]))
    ...
    def draw(self):
        ...
        #subprocess.check_output(['gzip', '-f', self.drawout_file])
        ...
        draws = draws[['location_id','year_id','age_group_id','sex_id','measure_id']+draw_cols]
        draws80 = draws[draws.age_group_id>21].groupby([
            'location_id','year_id','sex_id','measure_id']).mean().reset_index()
        draws80['age_group_id'] = 21
        draws = draws.append(draws80)
        ...
        draws_agestd = draws_agestd.groupby(['location_id','year_id',
            'sex_id','measure_id']).sum().reset_index()
