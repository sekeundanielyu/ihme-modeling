...
def read_data_pred_file(fdef):
    ...
    f = '{od}/{mv}/{cv}/locations/{l}/outputs/{s}/{y}/post_data_pred.csv'.format(
        od=outdir, cv=cv, mv=mvid, l=l, s=s, y=y)
    fadj = '{od}/{mv}/{cv}/locations/{l}/outputs/{s}/{y}/post_data_pred.csv'.format(
        od=outdir, cv=cv, mv=mvid, l=l, s=s, y=y)
    try:
        print 'Reading %s' % f
        df = pd.read_csv(f)
...
def compile_dp_files(mvid):
    ...
    # Identify location leaves
    lsvid = mvm['location_set_version_id'].values[0]
    loctree = lt.construct(lsvid)
    leaf_ids = [l.id for l in loctree.leaves()]
...
def calc_rmses(df):
    rmses = df.groupby(['integrand','cv_id','hold_out']).apply(lambda x:
        np.sqrt(
...
def calc_mean_errors(df):
    means = df.groupby(['integrand','cv_id','hold_out']).apply(lambda x:
            np.mean((np.log(x['adjust_median'])-np.log(x['pred_median']))**2))
...
def write_fit_stats(mvid, outdir, joutdir):
    ...
    try: os.makedirs(joutdir)
    except: pass
    try: os.chmod(joutdir, 0o775)
    except: pass
