...
// Set directory paths
	local path "strPath/cvd_pvd"
	local out_path "strPath"
...
// Start log  
	capture log close
	log using strPath/log_`location', replace
...
//Import and Compile Claudication Proportion (percent of people with PVD who show symptoms of claudication) from DisMod estimates
	local meid_claudication 1861
	get_draws, gbd_id_field(modelable_entity_id) gbd_id(`meid_claudication') location_ids(`location') measure_ids(18) status(best) source(dismod) clear
	forvalues i = 0/999 {
...
// Saving check file for summary statistics
	...
	save "path/check_`location'.dta", replace
...
// Save output file for upload with save results	
	...
	foreach kind of local save_type {
		...
		forvalues i = 0/999 {
			quietly rename `kind'_`i' draw_`i'
