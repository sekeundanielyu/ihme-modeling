...
cap log using "strPath/logs/prep_upload_ihd_csmr.smcl", replace
...
tempfile ihd_csmr_prep
save `ihd_csmr_prep', replace emptyok

foreach location of global location_ids {
	capture use "csmr_ihd_`location'.dta", clear
			if _rc {
			}
			else {
				append using `ihd_csmr_prep', force
				save `ihd_csmr_prep', replace
				}
			}
	
use `ihd_csmr_prep', clear

// Format for upload
	gen row_num = ""
	replace modelable_entity_id = 3233
	...
	gen case_definition = "(1-MI/IHD)*IHD deaths"
	gen note_modeler = "custom IHD csmr"
...
save "strPath/ihd_csmr_forupload.dta", replace
export excel using "strPath/ihd_csmr_for_upload.xlsx", replace sheet("extraction") firstrow(variables)
