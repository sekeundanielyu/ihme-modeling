...
// PULL IN LOCATION_ID AND INCOME CATEGORY FROM BASH COMMAND
	...
	capture log close
	log using strPath/log_asymp_`location', replace

	local prop_dir "strPath"
...
get_draws, gbd_id_field(modelable_entity_id) gbd_id(3233) source(dismod) location_ids(`location') measure_ids(5) clear
