...
** Setup to run on either cluster or local machine
	...
	local pre_clean "$prefix/WORK/05_risk/risks/drugs_alcohol/data/exp/inputs"
...
* Create complete country-year template 
	...
	quietly include "$prefix/WORK/10_gbd/00_library/functions/get_location_metadata.ado"
...
** Include education data for spacetime
	use "$prefix/WORK/01_covariates/02_inputs/education/update_2013/model/AdultOver25Education_IHME_1950-2020_Y2013M12Y16.dta", clear
	keep if educ_25plus != .
...
** Read WHO data 
	use "`pre_clean'/alcohol per capita consumption, 1960-2013.dta", clear
	...
	**Add on iso3
	import delimited "`pre_clean'/WHO_country_names.csv", clear 
...
**Read FAO data
	use "`pre_clean'/fao_fbs_1961-2011.dta", clear
	rename (country year) (countryname year_id)
	/* insheet using "$prefix/DATA/FAOSTAT/FAOSTAT_1961_2011_FOOD_BALANCE_SHEETS_ALL_Y2014M10D06.CSV"
	rename (country year) (countryname year_id)
	
	drop if year < 1970   
	Keep Domestic Supply Quantity (in 1000 tonnes) of alcohol
	keep if inlist(itemcode, 2655, 2656, 2657, 2658) 
	** codes for the following items: "Beverages, Alcoholic"; "Beverages, Fermented"; "Beer"; "Wine"
	keep if elementcode == 5301 
	** code for domestic supply quantity */

** Merge ISO3 identifiers
	merge m:1 countryname using "$prefix/Usable/Common Indicators/Country Codes/countrycodes_official.dta", keep(3) nogen
...
** Calculate densities for different alcoholic beverages (assume density of ethanol is 0.789 kg per liter, water is 1.000 kg per liter)
	...
	** Beverages, fermented
	*local den_fermented = 0.1*0.789 + 0.9
	** Beverages, alcoholic 
...
** Convert to liters and mulitply by % alcohol to get Liters pure alcohol per capita
	...
	*replace FAO_alcohol_lpc = value_kg_pc / `den_fermented' * 0.10 if item=="Beverages, Fermented"
...
*Run loess to calculate variance using a 5-year window. 
...
**Use loess estimates as mean to generate variance
...
save "J:\WORK\05_risk\risks\drugs_alcohol\data\exp\inputs\drugs_alcohol_average_lpc.dta", replace
