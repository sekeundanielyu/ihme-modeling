...
class Smoother:
	...
	def space_weights(self, loc, level, zeta_threshold):
		...
		# Generate vector of space weights using either zeta or zeta_no_data
	...
	def calculate_mad(self):
		...
		flatmap = pd.read_csv("/home/j/WORK/01_covariates/common/ubcov_central/_modules/04_model/_db/level_hierarchy.csv")
		flatmap = flatmap[['location_id', 'level_0', 'level_1', 'level_2', 'level_3']]
		flatmap = flatmap.rename(columns= {'location_id':'leaf_node'})
		merged = pd.merge(
				self.dataset, flatmap, left_on=self.spacevar, right_on='leaf_node',
				how='left')

		# Calculate residuals
		merged['st_resid'] = merged[self.datavar] - merged['st']
