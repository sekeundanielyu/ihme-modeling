...
if parallel == 1:
    loc_start = int(sys.argv[4])
    loc_end = int(sys.argv[5])
...
locs = pd.read_hdf('param.h5', 'location_hierarchy')

# Load data
data = pd.read_hdf('temp.h5', 'prepped')

# Load prior
prior = pd.read_hdf('temp.h5', '/prior/ELT1')
...
## Save
if parallel == 1:
    if not os.path.exists('st_temp/'):
        os.makedirs('st_temp/', 0777)
    df.to_csv('st_temp/%s_%s.csv' %(loc_start, loc_end), index=False)
else:
    df.to_hdf('temp.h5', 'st', mode='a', format='fixed')
