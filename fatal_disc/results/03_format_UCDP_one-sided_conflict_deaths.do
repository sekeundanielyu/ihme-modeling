// Purpose:	Format UCDP one-sided conflict deaths file downloaded from http://www.pcr.uu.se/research/ucdp/datasets/ucdp_one-sided_violence_dataset
...
// bring in the UCDP database
      import delimited "UCDP_ONE_SIDED_VIOLENCE_DATASET_1989_2014_V1_4_2015_Y2015M11D03.csv", clear
...
// replace Africa deaths where > 1 country is present using the UCDP Africa dataset; Region = 4 = Africa
	...
	// bring in Africa dataset
		...
		rename (side_a conflict_id year GWNOLoc country_name) (ActorName ActorId Year GWNoLoc Location)
		...
		// Append UCDP and Africa dataset observations 
			...
			// This will check whether UCDP dataset Best/Low/High numbers match up with the sum of the Africa dataset numbers
				...
				// 1 instance where there is a big discrepancy between Africa and UCDP dataset (Year: 1994; ActorId: 517) 
					// For this instance, UCDP number is a lot higher. Keep the UCDP number, but distribute the deaths based on how the Africa dataset is distributed.
...
// replace the location id (GWNoLoc) with the correct location, given that we separated out the deaths by location in each conflict-dyad-year
	...
	drop GWNoLoc1 GWNoLoc2 GWNoLoc3 indic nn
...
// Add a cause based on IsGovernmentActor field
	gen cause = ""
	replace cause = "legal intervention" if IsGovernmentActor == 1
	replace cause = "legal intervention" if regexm(ActorName, "Government")
...
// save final dataset
	saveold "UCDP_onesided_Africa.dta", replace
