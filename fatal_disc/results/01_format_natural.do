** Purpose: format EMDAT natural disaster data, taking into account length of disaster
...
** bring in online supplement data
	...
	** Philippines
	import excel "phl_death_count_investigation.xlsx", firstrow clear sheet( "Sheet1")
	keep if type == "disaster"
...
** bring in EMDAT data
	...
	** merge on iso3 codes
	rename country_name countryname
		...
		** some automated changes
		...
		replace countryname = "North Korea" if countryname == "Korea Democratic People's Republic" | iso == "PRK"
		replace countryname = "South Korea" if countryname == "Korea Republic" | iso == "KOR"
		...
		replace countryname = "Saint Vincent and the Grenadines" if countryname == "St. Vincent and the Grenadines" | countryname == "St. Vincent and the Grenad"
		...
		replace countryname = "Israel" if countryname == "Palenstine (West Bank)"
			...
			if "`makecomparison'" == "yes" {
				...
				di in red "There are `r(N)' country-years with a >=1% difference between new and currently used TOTAL death numbers"
					...
					** graph
					...
					pdfstart using "compare_new_old_disaster_deaths_after_redistrib_of_multiyrs.pdf", distexe("`acrobat_distiller_path'")
		...
		** reshape the data so that each line is an event-region
		...
		foreach char in 09 10 11 12 13 {
			...
			*****************************************************************************
			...
			replace location = "Inner Mongolia" if inlist(location, "Chifeng", "Guyangcounty (Inner Mongolia)", "Hohnut", "Hulunbeir", "InnerMOngolia", "InnerMongolia", "InnerMongolia Autonomous Region (Nei Mongol Zizhiqu)", "InnerMongolia Autonomous region") | inlist(location, "InnerMongolia autonomous region", "InnerMongolia region (Nei Mongol Zizhiqu)", "InnerMongolian Autonomous region", "Lindong(Balin-Zuo Qi)-Tianshan area (Inner Mongolia)", "Tongliao", "Ulanchabu(Chinese Autonomous Region of Inner Mongolia)", "XiliaoHe", "Xilingol") | inlist(location, "Xingan", "includingBairin Right Banner (Chifeng efecture)")
			...
			replace location = "Jiangxi" if inlist(location, "Jiangx", "BetweenRuichang and Jiujiang (Jiangxi )", "Ganzhu", "Guangchangcounty (Jiangxi )", "Jaingxi", "Jian", "Jianggxis", "Jiangxi") | inlist(location, "Jiangxis", "Jianxi", "Jiujiang(Jiangxi )", "Jiangxi s", "Lyangarea (Jiangsu )")
			...
			*********************************************************************
			...
			replace location = "Michoacán de Ocampo" if location == "Angangueo"
			replace location = "Colima" if location == "Armeria"
			replace location = "Michoacán de Ocampo" if location == "Arteaga"
			replace location = "Coahuila" if location == "Arteaga (Coahuila State" | location == "Arteaga (Coahuila state"
			replace location = "Michoacán de Ocampo" if location == "Arteagaarea (Michoacan)" | location == "Arteaga area (Michoacan)"
			replace location = "México" if location == "Atizapen de Zaragoza (Mexico State)" | location == "Atizapen de Zaragoza (Mexico state)"
			...
			replace location = "Michoacán de Ocampo" if (location == "CentralMexico" | location == "Central Mexico")
			replace location = "México" if location == "Chalcovalley (near Mexico City)" | location == "Chalco valley (near Mexico City)"
			...
			replace location = "Nuevo León" if location == "City of Monterrey" | location == "City Of Monterrey"
			...
			replace location = "México" if location == "Coacalco"
			...
			expand 2 if location == "Coahuila and Nuevo Leon states)"
				...
				replace location = "Nuevo León" if location == "Coahuila and Nuevo Leon states)" & mult == 2
				drop mult
			...
			replace location = "México" if location == "Cuantitlan"
			...
			replace location = "México" if location == "Ecateped"
			...
			replace location = "Michoacán de Ocampo" if location == "LazaroCardenas (Michoacan state)" | location == "Lazaro Cardenas (Michoacan state)"
			...
			replace location = "Michoacán de Ocampo" if location == "Michpacan"
			replace location = "Veracruz de Ignacio de la Llave" if location == "Minatitlan"
			replace location = "Nuevo León" if location == "Monterrey"
			...
			replace location = "Nuevo León" if location == "Nuevo Mexico"
			replace location = "Nuevo León" if location == "NuevoLeon"
			...
			replace location = "Michoacán de Ocampo" if location == "Patzcuaro"
			...
			expand 2 if location == "Popocatepelt"
				...
				drop mult
			replace location = "San Luis Potosí" if location == "Potosi"
			...
			expand 4 if location == "Puerto Vallarta (Jalisco state); Manzanillo (Colima state); Michoacan state; Nayarit state"
				...
				replace location = "Michoacán de Ocampo" if location == "Puerto Vallarta (Jalisco state); Manzanillo (Colima state); Michoacan state; Nayarit state" & mult == 3
			...
			replace location = "San Luis Potosí" if location == "San Luis"
			replace location = "San Luis Potosí" if location == "San Luis POtosi"
			replace location = "San Luis Potosí" if location == "San Luis Potosi"
			replace location = "San Luis Potosí" if location == "San Luis Potosi states"
			replace location = "San Luis Potosí" if location == "SanLuis Potosi"
			replace location = "San Luis Potosí" if location == "SanLuis Potosi"
			replace location = "Nuevo León" if location == "Sierrade la Marta" | location == "Sierra de la Marta"
			...
			replace location = "Yucatán" if location == "Tizimin (Merida satte"
			...
			replace location = "México" if location == "Tlenepantlade Baz" | location == "Tlenepantla de Baz"
			replace location = "Querétaro" if location == "Toliman"
			...
			replace location = "México" if location == "Tultitlan (Mexico city"
			...
			replace location = "Yucatán" if location == "Yucata peninsula"
			replace location = "Yucatán" if location == "Yucatan"
			replace location = "Yucatán" if location == "YucatanPeninsula" | location == "Yucatan Peninsula"
			...
			** Not in India
			...
			drop if location == "Shovanoliunion of Assasuni Upazila"	// Bangladesh

			preserve
			keep if iso3 == "IND"
			
			replace location = "Uttar Pradesh, Urban" if location == "Agra"
			...
			replace location = "Uttar Pradesh, Urban" if location == "Ajamgarh"
			replace location = "Maharashtra" if location == "Akoladistricts (Maharasthra state); Rewa"
			replace location = "Himachal Pradesh" if location == "Akpaand Kharo. Shimla"
			replace location = "Kerala, Urban" if location == "Alappuzha"
			replace location = "Kerala" if location == "Alapuzzhadistricts (Kerala state)"
			replace location = "Uttar Pradesh, Urban" if location == "Aligadh"
			replace location = "Uttar Pradesh, Urban" if location == "Aligarh"
			replace location = "Telangana" if location == "Alilabad districts (Andhra Pradesh state)" | location == "Alilabaddistricts (Andhra Pradesh state)"
			replace location = "Madhya Pradesh, Urban" if location == "Alirajpur districts (Madhya Pradesh state); Ajmer" | location == "Alirajpurdistricts (Madhya Pradesh state); Ajmer"
			replace location = "Uttar Pradesh, Urban" if location == "Allahabad"
			replace location = "Uttar Pradesh, Urban" if location == "Allahbad"
			replace location = "Uttarakhand, Urban" if location == "Almora"
			replace location = "Uttarakhand, Urban" if location == "Almora and Pithoragarh) Level 1 = Uttar Pradesh" | location == "Almoraand Pithoragarh) Level 1 = Uttar Pradesh"
			replace location = "Rajasthan, Urban" if location == "Alwar"
			replace location = "Bihar, Urban" if location == "Amarpur"
			replace location = "Uttar Pradesh" if location == "AmbedkarNagar"
			...
			replace location = "Andhra Pradesh, Urban" if location == "Ananatapur"
			replace location = "Gujarat, Urban" if location == "Anand"
			replace location = "Andhra Pradesh, Urban" if location == "Anantapur"
			replace location = "Andhra Pradesh, Urban" if location == "Anantapur and Kadpa (Andhra Pradesh)" | location == "Anantapurand Kadpa (Andhra Pradesh)"
			...
			replace location = "Andhra Pradesh" if location == "Andra Pradesh"
			replace location = "Andhra Pradesh" if location == "AndhraPradesh states"
			replace location = "Andhra Pradesh" if location == "AndhraPradesh)"
			replace location = "Andhra Pradesh" if location == "AndhraPradresh states"
			replace location = "Andhra Pradesh" if location == "AndhraPradesh" | location == "AndraPradesh"
			replace location = "Andhra Pradesh" if location == "Andra Pradesh" | location == "Andrha Pradesh"
			replace location = "Andhra Pradesh" if location == "AndrhraPradesh (Vishakhapatnam"
			...
			replace location = "Arunachal Pradesh, Rural" if location == "Anini"
			replace location = "Madhya Pradesh" if location == "Anuppur"
			replace location = "Bihar" if location == "Araria"
			replace location = "Arunachal Pradesh" if location == "Arunachal"
			replace location = "Arunachal Pradesh" if location == "Arunachal Pradesh"
			replace location = "Arunachal Pradesh" if location == "ArunachalPradesh and Island of Majuli state"
			replace location = "Arunachal Pradesh" if location == "ArunachalPradesh states"
			replace location = "Arunachal Pradesh" if location == "ArunachalPradesh)"
			replace location = "Arunachal Pradesh" if location == "ArunachalPradesh"
			replace location = "Bihar, Rural" if location == "Arwal"
			replace location = "Madhya Pradesh" if location == "Ashoknagar"
			...
			expand 3 if location == "Atharmalik(Angul district) (Jajpur district) - Orissa state. Chayyisgarh state and Madhya Pradesh state"
				bysort event_id location: gen mult = _n
				replace location = "Orissa, Rural" if location == "Atharmalik(Angul district) (Jajpur district) - Orissa state. Chayyisgarh state and Madhya Pradesh state" & mult == 1
				replace location = "Chhattisgarh" if location == "Atharmalik(Angul district) (Jajpur district) - Orissa state. Chayyisgarh state and Madhya Pradesh state" & mult == 2
				replace location = "Madhya Pradesh" if location == "Atharmalik(Angul district) (Jajpur district) - Orissa state. Chayyisgarh state and Madhya Pradesh state" & mult == 3
				drop mult
			replace location = "Uttar Pradesh" if location == "Auraiya"
			replace location = "Maharashtra, Urban" if location == "Aurangabad"
			replace location = "Uttar Pradesh, Urban" if location == "Azamgarh"
			replace location = "Uttar Pradesh, Urban" if location == "Azamgarharea (Uttar Pradesh)"
			replace location = "Uttar Pradesh" if regexm(location, "Badaun")
			replace location = "Uttar Pradesh" if location == "Badayu"
			replace location = "Madhya Pradesh, Urban" if location == "Badwani"
			...
			replace location = "Madhya Pradesh" if location == "Bagh"
			replace location = "Uttar Pradesh, Urban" if location == "Bahraich"
			replace location = "Assam" if location == "Baksa"
			replace location = "Assam" if location == "Baksa(Assam)"
			replace location = "Madhya Pradesh" if location == "Balaghat"
			replace location = "Madhya Pradesh" if location == "Balaghat(Madhya Pradesh)"
			...
			replace location = "Uttar Pradesh" if location == "Balia"
			...
			replace location = "Uttar Pradesh" if location == "Ballia"
			replace location = "Uttar Pradesh" if location == "Balrampur"
			...
			replace location = "Uttar Pradesh, Urban" if location == "Banda"
			...
			replace location = "Andhra Pradesh, Urban" if location == "Banswada"
			replace location = "Uttar Pradesh" if location == "Barabanki"
			replace location = "Uttar Pradesh" if location == "Barabanki(Uttar Pradesh)"
			replace location = "Orissa, Urban" if location == "Baragarh"
			replace location = "Uttar Pradesh, Urban" if location == "Baraich"
			replace location = "Jammu and Kashmir, Urban" if location == "Baramula"
			replace location = "Uttar Pradesh, Urban" if location == "Bareilly"
			replace location = "Uttar Pradesh, Urban" if location == "Bareli"
			...
			replace location = "Uttar Pradesh" if location == "Basti"
			...
			expand 4 if location == "Bayof Bengal"
				...
				replace location = "Andhra Pradesh" if location == "Bayof Bengal" & mult == 2
			...
			replace location = "Madhya Pradesh" if location == "Betuldistrict (Madhya Pradesh state)"
			...
			replace location = "Madhya Pradesh" if location == "Bhind"
			...
			replace location = "Uttar Pradesh" if location == "Bijnor"
			...
			expand 2 if location == "Bilaspurand Mandi districts (Himachal Pradesh)"
				bysort event_id location: gen mult = _n
				replace location = "Chhattisgarh" if location == "Bilaspurand Mandi districts (Himachal Pradesh)" & mult == 1
				replace location = "Chhattisgarh" if location == "Bilaspurand Mandi districts (Himachal Pradesh)" & mult == 2
				drop mult
			...
			replace location = "Uttar Pradesh, Urban" if location == "Bulandshahar"
			...
			replace location = "Madhya Pradesh, Urban" if location == "Burhanpur"
			...
			replace location = "Himachal Pradesh" if location == "Chamba"
			replace location = "Uttarakhand" if location == "Chamoli"
			replace location = "Uttar Pradesh" if location == "Chandauli"
			replace location = "Uttar Pradesh" if location == "Chandaulidistricts (Uttar Pradesh)"
			...
			replace location = "Arunachal Pradesh" if location == "Changlang"
			replace location = "Madhya Pradesh, Urban" if location == "Chattarpur"
			replace location = "Tamil Nadu, Urban" if location == "Chennai"
			replace location = "Madhya Pradesh" if location == "Chhattarpur"
			...
			replace location = "Uttar Pradesh, Urban" if location == "Chirgaon"
			replace location = "Karnataka" if location == "Chitradurga"
			replace location = "Madhya Pradesh, Urban" if location == "Chitrakut"
			replace location = "Andhra Pradesh" if location == "Chittoor"
			...
			replace location = "Andhra Pradesh, Urban" if location == "Cuddapah"
			...
			replace location = "Madhya Pradesh" if location == "Damoh"
			...
			replace location = "Madhya Pradesh" if location == "Datia"
			...
			expand 2 if (location == "Demajidistrict(Eastern Assam); western parts Meghalaya" | location == "Demajidistrict (Eastern Assam); western parts Meghalaya")
				...
				drop mult
			replace location = "Uttar Pradesh, Rural" if location == "Deogarh" | location == "Deoghar"
			replace location = "Uttar Pradesh" if location == "Deoria"
			replace location = "Madhya Pradesh, Urban" if location == "Dewas"
			...
			replace location = "Madhya Pradesh" if location == "Dindori"
			...
			replace location = "Andhra Pradesh" if location == "EastGavari"
			replace location = "Andhra Pradesh" if location == "EastGodavari"
			replace location = "Jharkhand" if location == "EastSinghbhum"
			replace location = "Andhra Pradesh" if location == "Eastand West Godavari"
			replace location = "Andhra Pradesh" if location == "EastGodavari"
			...
			replace location = "Uttar Pradesh" if location == "Etah"
			replace location = "Uttar Pradesh" if location == "Etawah"
			replace location = "Uttar Pradesh, Urban" if location == "Faizabad"
			replace location = "Uttar Pradesh, Urban" if location == "Farrukabad"
			replace location = "Uttar Pradesh, Urban" if location == "Farrukkhabad"
			replace location = "Uttar Pradesh" if location == "Fatehpur"
			replace location = "Uttar Pradesh" if location == "Firozabad"
			...
			replace location = "Uttar Pradesh" if location == "Gajipur"
			...
			expand 3 if location == "Gangavaram(Coast of Andhra Pradesh); South Odisha; Chhattisgarh"
				bysort event_id location: gen mult = _n
				replace location = "Andhra Pradesh, Rural" if location == "Gangavaram(Coast of Andhra Pradesh); South Odisha; Chhattisgarh" & mult == 1
				replace location = "Orissa" if location == "Gangavaram(Coast of Andhra Pradesh); South Odisha; Chhattisgarh" & mult == 2
				replace location = "Chhattisgarh" if location == "Gangavaram(Coast of Andhra Pradesh); South Odisha; Chhattisgarh" & mult == 3
				drop mult
			...
			replace location = "Uttar Pradesh" if location == "GautamBudh Nagar"
			replace location = "Bihar, Urban" if location == "Gaya"
			replace location = "Uttar Pradesh, Urban" if location == "Ghaziabad"
			replace location = "Uttar Pradesh, Urban" if location == "Ghaziabad (Utar Pradesh)" | location == "Ghaziabad(Utar Pradesh)"
			replace location = "Uttar Pradesh, Urban" if location == "Ghazipur"
			...
			expand 4 if location == "Golfedu Bengale"
				...
				replace location = "Andhra Pradesh" if location == "Golfedu Bengale" & mult == 2
				...
				drop mult
			replace location = "Uttar Pradesh" if location == "Gonda"
			replace location = "Maharashtra" if location == "Gondia"
			replace location = "Bihar" if location == "Gopalganj"
			replace location = "Uttar Pradesh, Urban" if location == "Gorakhpur"
			...
			replace location = "Madhya Pradesh" if location == "Guna"
			replace location = "Andhra Pradesh" if location == "Guntardistricts (Andhra Pradesh)"
			replace location = "Andrha Pradesh" if location == "Guntoor(Guntur) and Prakasham districts (Andhra Pradesh)"
			replace location = "Andhra Pradesh" if location == "Guntur"
			replace location = "Andhra Pradesh" if location == "Gunturdistrits (Andhra Pradesh state); Tamil Nadu state"
			replace location = "Madhya Pradesh, Urban" if location == "Gwalior"
			replace location = "Assam" if location == "Hailakandi"
			replace location = "West Bengal, Urban" if location == "Haldibari"
			replace location = "Himachal Pradesh" if location == "Hamirpur"
			replace location = "Rajasthan" if location == "Hanumangarh"
			replace location = "Uttar Pradesh" if location == "Hardoi"
			...
			replace location = "Uttar Pradesh, Urban" if location == "Hathras"
			...
			replace location = "Himachal Pradesh" if location == "HimachaclPradesh"
			replace location = "Himachal Pradesh" if location == "HimachelPradesh"
			replace location = "Himachal Pradesh" if location == "Himachal"
			replace location = "Himachal Pradesh" if location == "HimachalPradesh"
			replace location = "Himachal Pradesh" if location == "HimachalPradesh states"
			replace location = "Himachal Pradesh" if location == "HimachalPradesh)"
			replace location = "Himachal Pradesh" if location == "HimachalPradesh"
			replace location = "Himachal Pradesh" if location == "Himalayanstate of Himachal Pradesh."
			...
			replace location = "Madhya Pradesh, Urban" if location == "Hopal"
			replace location = "Madhya Pradesh, Urban" if location == "Hoshangabad"
			...
			replace location = "Telangana, Urban" if location == "Hyderabad(Andhra Pradesh)"
			...
			replace location = "Karnataka, Urban" if location == "JPNagar"
			replace location = "Madhya Pradesh, Urban" if location == "Jabalpur(Madhya Pradesh)" | location == "Jabalpur"
			expand 3 if location == "Jabalpur(Mahya Pradesh); Uttar Prasdesh; Assam"
				bysort event_id location: gen mult = _n
				replace location = "Madhya Pradesh" if location == "Jabalpur(Mahya Pradesh); Uttar Prasdesh; Assam" & mult == 1
				replace location = "Uttar Pradesh" if location == "Jabalpur(Mahya Pradesh); Uttar Prasdesh; Assam" & mult == 2
				replace location = "Assam" if location == "Jabalpur(Mahya Pradesh); Uttar Prasdesh; Assam" & mult == 3
				drop mult
			replace location = "Madhya Pradesh" if location == "Jabalpurand Seoni districts (Madhya Pradesh state)"
			...
			replace location = "Uttar Pradesh" if location == "Jalaun"
			...
			replace location = "Uttar Pradesh, Urban" if location == "Jaunpur"
			replace location = "Bihar" if location == "Jehanabad"
			replace location = "Madhya Pradesh, Urban" if location == "Jhabua"
			replace location = "Uttar Pradesh" if location == "Jhansi"
			replace location = "Uttar Pradesh" if location == "Jhapadistrict"
			...
			replace location = "Uttar Pradesh" if location == "JyotibaphuleNagar"
			replace location = "Karnataka, Rural" if location == "Kabini(Karnataka)"
			replace location = "Gujarat, Urban" if location == "Kachch-Bhuj"
			replace location = "Andhra Pradesh, Urban" if location == "Kadapa"
			replace location = "Andhra Pradesh, Urban" if location == "Kadapaand Nellore districts (Andhra Pradesh)"
			...
			replace location = "Andhra Pradesh, Urban" if location == "Kakinada(Andhra Pradesh state)"
			replace location = "Orissa" if location == "Kalahandi"
			replace location = "Himachal Pradesh, Rural" if location == "Kalpaand Yangthang districts"
			...
			replace location = "Himachal Pradesh, Urban" if location == "Kangra"
			expand 3 if location == "Kannadadistricts (Karnataka state); Maharashtra state; Kurnool"
				...
				replace location = "Andhra Pradesh, Urban" if location == "Kannadadistricts (Karnataka state); Maharashtra state; Kurnool" & mult == 3
				drop mult
			replace location = "Uttar Pradesh, Urban" if location == "Kannauj"
			replace location = "Tamil Nadu" if location == "Kanniyakumari"
			replace location = "Uttar Pradesh, Urban" if location == "Kannoj"
			replace location = "Kerala, Urban" if location == "Kannur"
			replace location = "Uttar Pradesh, Urban" if location == "Kanpur"
			replace location = "Uttar Pradesh, Urban" if location == "Kanpur(Uttar Pradesh)"
			replace location = "Uttar Pradesh" if location == "KanpurDehat"
			replace location = "Uttar Pradesh" if location == "KanpurNagar"
			...
			replace location = "Himachal Pradesh, Rural" if location == "Karcham"
			...
			replace location = "Uttar Pradesh" if location == "KashiramNagar"
			replace location = "Jammu and Kashmir" if location == "Kashmir"
			replace location = "West Bengal, Rural" if location == "Kasiabari"
			replace location = "Uttar Pradesh" if location == "KasiramNagar"
			replace location = "Bihar" if location == "Katihar"
			replace location = "Madhya Pradesh" if location == "Katni"
			replace location = "Uttar Pradesh" if location == "Kaushambi"
			...
			replace location = "Kerala" if location == "Keralastate)"
			replace location = "Himachal Pradesh, Rural" if location == "Khab"
			...
			replace location = "Telangana, Urban" if location == "Khammam)of Andhra Pradesh"
			replace location = "Madhya Pradesh" if location == "Khandwa"
			...
			replace location = "Madhya Pradesh, Rural" if location == "Khiri"
			...
			replace location = "Himachal Pradesh" if location == "Kinnaur"
			...
			replace location = "Andhra Pradesh" if location == "Krishna"
			replace location = "Andhra Pradesh" if location == "Krishna(Andhra Pradesh)"
			replace location = "Andhra Pradesh" if location == "Krishnadistricts (Andhra Pradesh)"
			replace location = "Andhra Pradesh" if location == "Krishnagirin"
			replace location = "Himachal Pradesh" if location == "Kullu"
			replace location = "West Bengal, Urban" if location == "Kultali"
			replace location = "Himachal Pradesh" if location == "Kulu"
			replace location = "Himachal Pradesh" if location == "Kulu(Himachal Pradesh)"
			replace location = "Andhra Pradesh, Urban" if location == "Kurnool"
			replace location = "Uttar Pradesh" if location == "Kushinagar"
			replace location = "Gujarat" if location == "Kutch"
			replace location = "Himachal Pradesh" if location == "Lahauland Spiti"
			...
			replace location = "Himachal Pradesh, Rural" if location == "Leo"
			replace location = "Jharkhand" if location == "Lohardaga"
			replace location = "Arunachal Pradesh" if location == "Lohitand Anjaw"
			replace location = "Nagaland" if location == "Longleng"
			replace location = "Manipur" if location == "Lourembam"
			replace location = "Uttar Pradesh, Urban" if location == "Lucknow"
			replace location = "Mizoram" if location == "Lunglei"
			replace location = "Bihar" if location == "Madhbani"
			replace location = "Uttar Pradesh, Rural" if location == "Madhepura"
			replace location = "Uttar Pradesh" if location == "Madhubani"
			replace location = "Uttar Pradesh" if location == "Madhubanidistricts (Uttar Pradesh state)"
			replace location = "Madhya Pradesh" if location == "MadhyaPradesh"
			replace location = "Madhya Pradesh" if location == "MadhyaPrasdeh"
			replace location = "Madhya Pradesh" if location == "MadhyaPrasdesh"
			replace location = "Tamil Nadu, Urban" if location == "Madras"
			replace location = "Tamil Nadu, Urban" if location == "Madurai"
			replace location = "Madhya Pradesh" if location == "MadyhaPradesh"
			replace location = "Jharkhand, Urban" if location == "Mafhupur"
			replace location = "Madhya Pradesh" if location == "MafyaPradesh"
			replace location = "Andhra Pradesh, Urban" if location == "Mahaboobnagar"
			...
			replace location = "Uttar Pradesh" if location == "Mahoba"
			replace location = "Madhya Pradesh, Urban" if location == "Maihar(Madhya Pradesh)"
			replace location = "Uttar Pradesh, Urban" if location == "Mainpuri"
			replace location = "Uttar Pradesh, Urban" if location == "Mainpuri.(Uttar Pradesh) Saini"
			...
			replace location = "Himachal Pradesh, Urban" if location == "Mandi"
			replace location = "Madhya Pradesh, Urban" if location == "Mandla"
			...
			replace location = "Uttar Pradesh" if location == "Mathura"
			replace location = "West Bengal, Rural" if location == "Mathurapur"
			replace location = "Uttar Pradesh, Urban" if location == "Mau"
			...
			replace location = "Uttar Pradesh, Urban" if location == "Meerut"
			...
			replace location = "Uttar Pradesh, Urban" if location == "Merut"
			replace location = "West Bengal" if location == "MidnaporeEast and West"
			replace location = "Uttar Pradesh" if location == "Midnapurdis.- Level 1 = Uttar Pradesh"
			replace location = "Uttar Pradesh" if location == "Mirzapurdistricts (Uttar Pradesh state); Patna"
			...
			replace location = "Uttar Pradesh, Urban" if location == "Moradabad"
			replace location = "Madhya Pradesh, Urban" if location == "Morena"
			...
			replace location = "Uttar Pradesh, Urban" if location == "MujafferNagar"
			...
			replace location = "Uttar Pradesh, Urban" if location == "Muradabad"
			replace location = "West Bengal" if location == "Murshidabad"
			replace location = "Assam" if location == "Mushakpur"
			replace location = "Uttar Pradesh, Urban" if location == "Muzaffarnagar"
			...
			expand 3 if location == "Naintial(Uttarakhand state); Bihar state; Uttar Pradesh"
				bysort event_id location: gen mult = _n
				replace location = "Uttarakhand" if location == "Naintial(Uttarakhand state); Bihar state; Uttar Pradesh" & mult == 1
				replace location = "Bihar" if location == "Naintial(Uttarakhand state); Bihar state; Uttar Pradesh" & mult == 2
				replace location = "Uttar Pradesh" if location == "Naintial(Uttarakhand state); Bihar state; Uttar Pradesh" & mult == 3
				drop mult
			...
			replace location = "Andhra Pradesh" if location == "Nalgonda"
			replace location = "Andhra Pradesh" if location == "Nalgondadistricts (Andhra Pradesh state)"
			replace location = "Andhra Pradesh" if location == "Nalgondadistricts (Andhra Pradesh)"
			...
			replace location = "Madhya Pradesh" if location == "Narsinghpur"
			...
			replace location = "Uttar Pradesh, Urban" if location == "Nawabganj(Uttar Pradesh)"
			...
			replace location = "Andhra Pradesh, Urban" if location == "Nellore"
			replace location = "Andhra Pradesh, Urban" if location == "Nellore)"
			replace location = "Andhra Pradesh, Urban" if location == "Neloore"
			...
			replace location = "Uttar Pradesh, Urban" if location == "PalliaKalan"
			replace location = "Gujarat" if location == "Panchmahal"
			replace location = "Madhya Pradesh, Urban" if location == "Panna"
			...
			replace location = "Uttar Pradesh" if location == "Pilibhit"
			...
			replace location = "Andhra Pradesh" if location == "Prakasam"
			...
			replace location = "Uttar Pradesh, Urban" if location == "Raebareli"
			replace location = "Uttar Pradesh, Urban" if location == "RaiBareilly"
			...
			replace location = "Madhya Pradesh" if location == "Raisen"
			...
			replace location = "Uttar Pradesh, Urban" if location == "Rampur"
			replace location = "Uttar Pradesh, Urban" if location == "Rampur.Kinnaur region: Sangla"
			...
			replace location = "Madhya Pradesh" if location == "Ratlam"
			...
			replace location = "Madhya Pradesh" if location == "Rewa"
			replace location = "Madhya Pradesh" if location == "Rivaand Katni. Satna"
			replace location = "Bihar" if location == "Rohtas"
			replace location = "Arunachal Pradesh" if location == "Roing& Along (Arunachal Pradesh state)"
			...
			replace location = "Madhya Pradesh" if location == "Sagar"
			replace location = "Uttar Pradesh" if location == "Saharanpur"
			...
			replace location = "Madhya Pradesh" if location == "Sahra"
			...
			replace location = "Uttar Pradesh" if location == "SantKabir Nagar"
			...
			replace location = "Madhya Pradesh, Urban" if location == "Satna"
			replace location = "Gujarat" if location == "Saurashtraregion (Gujarat)"
			replace location = "Rajasthan, Urban" if location == "SawaiMadhopur"
			replace location = "Madhya Pradesh, Urban" if location == "Sehore"
			replace location = "Manipur" if location == "Senapati"
			replace location = "Madhya Pradesh, Urban" if location == "Seoni"
			replace location = "Arunachal Pradesh" if location == "Seppa"
			replace location = "Madhya Pradesh" if location == "Shahdol"
			replace location = "Uttar Pradesh, Urban" if location == "Shahjahanpur"
			replace location = "Uttar Pradesh, Urban" if location == "Shahjahapur"
			replace location = "Madhya Pradesh, Urban" if location == "Shajapur"
			...
			replace location = "Madhya Pradesh" if location == "SheopurKalan"
			replace location = "Madhya Pradesh, Urban" if location == "Shivpuri"
			replace location = "Uttar Pradesh" if location == "Shravasti"
			replace location = "Uttar Pradesh" if location == "Shravasti(Uttar Pradesh state)"
			replace location = "Uttar Pradesh" if location == "Shrawasti"
			replace location = "Assam" if location == "Sibasagar"
			replace location = "Assam" if location == "Sibsagadistricts (Assam )"
			replace location = "Uttar Pradesh" if location == "SiddharthNagar"
			replace location = "Madhya Pradesh" if location == "Sidhi"
			...
			replace location = "Himachal Pradesh" if location == "Simla"
			replace location = "Madhya Pradesh" if location == "Singrauli"
			replace location = "Tamil Nadu" if location == "Sirkali"
			replace location = "Himachal Pradesh" if location == "Sirmaur"
			...
			replace location = "Uttar Pradesh, Urban" if location == "Sitapur"
			...
			replace location = "Himachal Pradesh" if location == "Solan"
			replace location = "Himachal Pradesh" if location == "Solang(near Kullu district"
			...
			replace location = "Andhra Pradesh" if location == "Srikakulam"
			replace location = "Tamil Nadu" if location == "Srirangam"
			replace location = "Orissa" if location == "SubarnapurCuttack"
			replace location = "Uttar Pradesh" if location == "Sultanpur"
			replace location = "Himachal Pradesh, Rural" if location == "Sumdo.Khaab"
			...
			replace location = "Madhya Pradesh" if location == "Tikamgarh"
			...
			replace location = "Madhya Pradesh" if location == "Umaria"
			expand 2 if location == "Unadistricts (Himachal Pradesh state); Chatra"
				bysort event_id location: gen mult = _n
				replace location = "Himachal Pradesh"  if location == "Unadistricts (Himachal Pradesh state); Chatra" & mult == 1
				replace location = "Jharkhand"  if location == "Unadistricts (Himachal Pradesh state); Chatra" & mult == 2
				drop mult
			replace location = "Uttar Pradesh" if location == "Unnao"
			replace location = "Uttar Pradesh" if location == "Unnav(Uttar Pradesh state)"
			replace location = "Jammu and Kashmir, Rural" if location == "Uri"
			replace location = "Uttar Pradesh" if location == "UttarPradesh" | location == "UttarPradesh state)"
			replace location = "Uttar Pradesh" if location == "UttarPradesh state"
			replace location = "Uttar Pradesh" if location == "UttarPradesh states"
			replace location = "Uttar Pradesh" if location == "UttarPradesh)"
			replace location = "Uttar Pradesh" if location == "UttarPradesh; Gonda"
			replace location = "Uttar Pradesh" if location == "Uttaradesh"
			...
			replace location = "Uttar Pradesh" if location == "UtterPradesh" | location == "Malpavillage (Pithoragrah district" | location == "Mansunavillage (Uttar Pradesh state)" | location == "Pithoragarhdistrict (Uttarakhand)"
			...
			replace location = "Uttar Pradesh, Urban" if location == "Varanasi"
			replace location = "Uttar Pradesh, Urban" if location == "Varanasi.Ankinghat"
			replace location = "Assam" if location == "Varpeta"
			replace location = "Tamil Nadu" if location == "Vellore"
			replace location = "Madhya Pradesh" if location == "Vidisha"
			replace location = "Tamil Nadu" if location == "Villupuram"
			replace location = "Andhra Pradesh" if location == "Visakhapatam"
			replace location = "Andhra Pradesh" if location == "Visakhapatanam"
			replace location = "Andhra Pradesh, Urban" if location == "Visakhapathnamcity"
			replace location = "Andhra Pradesh" if location == "Visakhapatnamdistrcits (Andhra Pradesh)"
			replace location = "Andhra Pradesh" if location == "Visakhapatnamdistricts (Andhra Pradesh)"
			replace location = "Andhra Pradesh" if location == "Vizianagaram"
			...
			replace location = "Andhra Pradesh" if location == "WesGovari"
			...
			replace location = "Andhra Pradesh" if location == "WestGodavari"
			replace location = "Andhra Pradesh" if location == "WestGodavari (Andhra Pradesh state)"
			...
			replace location = "Arunachal Pradesh" if location == "Yingkiong"
			expand 2 if location == "Zunhebotodistricts (Nagaland state) Ambedkar Nagar"
				...
				replace location = "Uttar Pradesh" if location == "Zunhebotodistricts (Nagaland state) Ambedkar Nagar" & mult == 2
				drop mult
			...
			** Unkown, unable to split
			...
			replace location = "Pará" if location == "Almerim"
			...
			replace location = "Amapa" if location == "Amapá"
			replace location = "Pará" if location == "Anajas (Para state)"
			...
			replace location = "São Paulo" if location == "Apiai (Sao Paulo state)"
			...
			replace location = "Bahia" if location == "Bahía"
			...
			replace location = "São Paulo" if location == "Cajati"
			expand 2 if location == "Campina do Simão) and Santa Catarina states"
				bysort event_id location: gen mult = _n
				replace location = "Paraná" if location == "Campina do Simão) and Santa Catarina states" & mult == 1
				replace location = "Santa Catarina" if location == "Campina do Simão) and Santa Catarina states" & mult == 2
				drop mult
			replace location = "Roraima" if location == "Canta"
			replace location = "Pará" if location == "Capitao Poco"
			replace location = "Roraima" if location == "Caracarai"
			replace location = "Minas Gerais" if location == "Caratinga"
			replace location = "Paraná" if location == "Cascavel"
			replace location = "Ceará" if location == "Ceara"
			replace location = "Ceará" if location == "Ceara states"
			replace location = "Piauí" if location == "Cocal (Piaui state)"
			replace location = "Minas Gerais" if location == "Contagem"
			replace location = "Santa Catarina" if location == "Criciuma"
			replace location = "Paraná" if location == "Cruz Machado"
			...
			replace location = "Paraná" if location == "Curitiba"
			replace location = "Rio de Janeiro" if location == "Duque de Caxias"
			replace location = "São Paulo" if location == "Eldorado (Sao Paulo state)"
			replace location = "Rio de Janeiro" if location == "Engenheiro Paulo De Frontin (Rio de Janeiro)"
			replace location = "Espírito Santo" if location == "Espirito Santo"
			replace location = "Espírito Santo" if location == "Espirito Santo (Jaguare"
			replace location = "Espírito Santo" if location == "Espirito Santo (Leopoldina)"
			replace location = "Espírito Santo" if location == "Espirito Santo state"
			replace location = "Espírito Santo" if location == "Espirito Santo states"
			...
			replace location = "São Paulo" if location == "Glicerio"
			replace location = "Paraná" if location == "Guarapuava"
			replace location = "São Paulo" if location == "Guaruja))"
			replace location = "São Paulo" if location == "Iguape"
			replace location = "Santa Catarina" if location == "Ilhota"
			replace location = "Ceará" if location == "Inhamuns"
			...
			replace location = "São Paulo" if location == "Itaoca"
			replace location = "Santa Catarina" if location == "Itapoa"
			replace location = "São Paulo" if location == "Itariri"
			...
			replace location = "São Paulo" if location == "Jacupiranga"
			replace location = "São Paulo" if location == "Jales"
			replace location = "Rio de Janeiro" if location == "Japari"
			replace location = "Santa Catarina" if location == "Jaragua do Sul"
			replace location = "São Paulo" if location == "Jiundiai (Sao Paulo state)"
			replace location = "São Paulo" if location == "Juquia"
			replace location = "Bahia" if location == "Lajedinho (Bahia state)"
			replace location = "Paraná" if location == "Laranjeiras do Sul"
			replace location = "Paraná" if location == "Las Palmas (Parana)"
			...
			replace location = "Paraná" if location == "Mallet"
			...
			replace location = "Pará" if location == "Maraba"
			replace location = "Pará" if location == "Marcaba"
			replace location = "Amapá" if location == "Maraca"
			replace location = "Maranhão" if location == "Maranhao"
			...
			replace location = "São Paulo" if location == "Maua)"
			replace location = "Paraná" if location == "Medianeira"
			...
			replace location = "São Paulo" if location == "Miracatu"
			...
			replace location = "Ceará" if location == "Near Fortaleza (Ceara state)"
			...
			expand 9 if location == "Nord East"
				bysort event_id location: gen mult = _n
				replace location = "Maranhão" if location == "Nord East" & mult == 1
				replace location = "Piauí" if location == "Nord East" & mult == 2
				replace location = "Rio Grande do Norte" if location == "Nord East" & mult == 3
				replace location = "Paraíba" if location == "Nord East" & mult == 4
			...
			expand 9 if location == "Nordeste"
				bysort event_id location: gen mult = _n
				replace location = "Maranhão" if location == "Nordeste" & mult == 1
				replace location = "Piauí" if location == "Nordeste" & mult == 2
				replace location = "Rio Grande do Norte" if location == "Nordeste" & mult == 3
				replace location = "Paraíba" if location == "Nordeste" & mult == 4
			...
			replace location = "Espírito Santo" if location == "Nova Venecia)"
			...
			replace location = "Paraíba" if location == "Paraiba" | location == "Paraiba state"
			replace location = "Paraná" if location == "Parana"
			replace location = "Paraná" if location == "Parana (Campo Largo"
			replace location = "Paraná" if location == "Parana State"
			replace location = "Paraná" if location == "Parana states"
			replace location = "São Paulo" if location == "Pariquera Acu"
			...
			replace location = "São Paulo" if location == "Peruibe"
			replace location = "São Paulo" if location == "Petropolis"
			replace location = "Rio de Janeiro" if location == "Piabanka river. Paulo de Fronty"
			replace location = "Piaui" if location == "Piaui" | location == "Piauí"
			replace location = "Piaui" if location == "Piaui State"
			replace location = "Piaui" if location == "Piaui states"
			replace location = "Paraná" if location == "Pinhão"
			...
			replace location = "Pará" if location == "Porto de Moz"
			replace location = "São Paulo" if location == "Queimados"
			...
			replace location = "Paraná" if location == "Reserva do Iguaçu"
			...
			replace location = "Espírito Santo" if location == "Rio Banal"
			replace location = "São Paulo" if location == "Rio Claro)"
			...
			replace location = "Rondônia" if location == "Rondonia"
			replace location = "Sau Paulo" if location == "San Pablo" | location == "Sao Paulo city  (Sao Paulo)"
			replace location = "São Paulo" if location == "San Paulo" | location == "(4) Sao Paulo"
			...
			replace location = "Maranhão" if location == "Sao Joao da Bastida"
			replace location = "São Paulo" if location == "Sao Jose do Rio Preto"
			replace location = "São Paulo" if location == "Sao Jose dos Campos"
			replace location = "São Paulo" if location == "Sao Jose dos Campos municipalities"
			replace location = "São Paulo" if location == "Sao Paulo" | location == "Sau Paulo"
			replace location = "São Paulo" if location == "Sao Paulo (Aracatuba"
			replace location = "São Paulo" if location == "Sao Paulo (Jundiai"
			replace location = "São Paulo" if location == "Sao Paulo (Santos"
			replace location = "São Paulo" if location == "Sao Paulo City"
			replace location = "São Paulo" if location == "Sao Paulo State(Sao Paulo city )"
			replace location = "São Paulo" if location == "Sao Paulo and Campos de Jordao cities. Towns: Volta Redonda"
			replace location = "São Paulo" if location == "Sao Paulo city (Sao Paulo)"
			replace location = "São Paulo" if location == "Sao Paulo state"
			replace location = "São Paulo" if location == "Sao Paulo states"
			...
			replace location = "Paraná" if location == "Sulinain"
			replace location = "Rio de Janeiro" if location == "Sumidaro"
			replace location = "Paraná" if location == "São Matheus do Sul"
			...
			************************************************************************
			...
			replace location = "Illinois" if location == "Rockford"
	...
	** make the dataset wide
		** first need to collapse the number killed by iso3-disaster year - type
		collapse (sum) numkilled, by(countryname iso3 location_id gbd_region disyear type _m)
		** split unknown subnationals
		preserve
			import excel using "disaster_SUB_strUser.xlsx", clear firstrow
			...
			replace countryname = "Uttar Pradesh" if countryname == "Uttarpradesh"
...
save "formatted_type_specific_disaster_deaths.dta", replace
save "formatted_type_specific_disaster_deaths_`date'.dta", replace
