...
// bring in file
	import excel "UCDP_GED_1989_2014_V_2_0_2015_SUB_MML.xlsx", clear firstrow

// keep on variables that may be needed and sum of death numbers
	collapse (sum) deaths_a deaths_b deaths_civilians deaths_unknown best_est high_est low_est, by( year type_of_violence conflict_dset_id conflict_name dyad_dset_id dyad_name side_a side_a_dset_id side_b side_b_dset_id country ihme_loc_id)
	...
	// generate a deaths varaible for best_estimate that takes into account the unkown deaths when >1 country is present
		...
		// if the best estimate doesn't match up to the total death (deada+deadb+deadciv+deaths_unknown), then replace the deaths estimate by multiplying the best deaths estimate with the ratio of deaths for sidea/b/civilians when >1 country is present
	...
	// Look at how deaths are distributed based on type of conflict when >1 country was involved
		...
		** Country name fixes
		...
		// Get ISO3
		odbc load, exec("SELECT ihme_loc_id, location_name FROM shared.location_hierarchy_history WHERE location_set_version_id = 39 AND location_type = 'admin0'") dsn(strConnection) clear
		...
		// collapse
		collapse (sum) dbest dlow dhigh, by( year type_of_violence conflict_dset_id conflict_name dyad_dset_id dyad_name side_a side_a_dset_id side_b side_b_dset_id iso3 country_name)
