...
// directories
local data_dir "`j'/WORK/04_epi/02_models/01_code/06_custom/neonatal/data/02_analysis/`acause'/prev_28_days/`me_id'"

*******************************************************************

// import data for this location-year-sex
di "Location = `location_id', year = `year_id', sex = `sex_id'"
get_draws, gbd_id_field(modelable_entity_id) gbd_id(`me_id') source(epi) measure_ids(5) location_ids(`location_id') year_ids(`year_id') age_group_ids(2 3) sex_ids(`sex_id') status(best) clear
