...
// set directories 	
	...
	local out_dir "`working_dir'/01_B_rhogam_adjustment"
...
// Create timestamp for logs
	...
	//log
	capture log close
	log using "`log_dir'/01_B_rhogam_adjustment_`timestamp'.smcl", replace
...
///////////////////////////////////////////////////////////// */
	...
	// prep: get NMR data ready (we use it as a scale to determine what countries do/do not get rhogam-adjusted)
		...
		tempfile nmr
		save `nmr'
		get_location_metadata, location_set_id(21) clear // location set 21 is for mortality computation; includes China (without Hong Kong and Macao)
		tempfile locations
		save `locations'
	...
	1. Rhogam Dose Data 
	//////////////// */
		local rhogam_dir = "`j'/WORK/04_epi/02_models/01_code/06_custom/neonatal/data/01_prep/neonatal_hemolytic/01_rh_disease/rhogam_temp.csv"
		...
		drop if level == 4 // We have Rhogam doses for Georgia (the country) so drop match to Gerogia (the US state)
	...
	/////////////////////////////////////////// */
		...
		local rh_prev_dir "`working_dir'/01_A_rh_prev/rh_incompatible_count_all_draws.dta"
	...
	// birth numbers.  The relationship is: adjusted pregnancies = rh-incompatible pregnancies * (1-rhogam_prop).
		...
		preserve
			...
			save "`out_dir'/rhogam_adjusted_pregnancies_all_draws.dta", replace
			export delimited using "`out_dir'/rhogam_adjusted_pregnancies_all_draws.csv", replace
		restore
		...
		export delimited using "`out_dir'/rhogam_adjusted_pregnancies_summary_stats.csv", replace
		save "`out_dir'/rhogam_adjusted_pregnancies_summary_stats.dta", replace
	...
	if `plot_adjustments'==1{
		...
		use "`out_dir'/rhogam_adjusted_pregnancies_summary_stats.dta", clear
		...
		pdfstart using "`plot_dir'/adjusted_pregnancies_time_series.pdf"
