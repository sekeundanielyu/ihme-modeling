...
// open data file
	insheet using "$CW_DATADIR/sf12_dw_survey_results.csv", comma names clear
...
// merge loginids (how we identified which health state they responded to) to get lay decriptions associated with them
	merge m:1 id using "$CW_DATADIR/lay_descriptions.dta", nogen

// merge on disability weights
	rename conditiondescription laydescriptions
	merge m:1 laydescriptions using "$CW_DATADIR/desc_id_merge.dta", keep(1 3) nogen //  _m=2 are just unused descriptions, we dont need them
	merge m:1 hhseqid using  "$CW_DATADIR/mean_DW", keep(1 3) nogen //  _m=2 are just unused descriptions, we dont need them
...
// the resulting file will be used in R to estimate the DW for each SF-12 score in survey datasets.
	save "$SAVE_DIR/1_crosswalk_data.dta", replace
