...
// This is a small dataset and w/o RE so can be done quickly on ones own windows machine, no need to parallelize on cluster.
...
use "$SAVE_DIR/2a_ahs_prepped_to_crosswalk.dta", clear
drop if key == -999
tempfile data
save `data'

// get data
use "$SAVE_DIR/2b_ahs_lowess_r_interpolation.dta", clear
drop if key == -999
...
foreach mo of numlist 12 {
	...
	label drop _all
	drop Imental_unipolar_mdd`opp' Imental_anxiety`opp' Imental_unipolar_dys`opp'  Imental_alcohol`opp' Imental_drug_other`opp'
	if `mo' == 1  drop Imental_other12
	...
	forvalues x = 0/999 {
		...
		foreach como of varlist I* {
			...
			cap mkdir "$SAVE_DIR/3a_ahs`mo'_bootstrap_datasets"
			cap mkdir "$SAVE_DIR/3a_ahs`mo'_bootstrap_datasets//`x'"
			save	  "$SAVE_DIR/3a_ahs`mo'_bootstrap_datasets//`x'//`como'", replace
		...
		cap mkdir "$SAVE_DIR/3a_ahs`mo'_dw_draws"
		save	  "$SAVE_DIR/3a_ahs`mo'_dw_draws//`x'.dta", replace
