################################################################################
## Date:        Originally written in May-June of 2012, updated in Oct-Nov 2012, updated February 2015
...
## bring in locations from database

  source(paste0(root, "get_locations.r"))
  codes <- get_locations()
...
# fix source so that Syria, Guyana, Korea, South Africa and its subnationals are treated as having two VR systems [Turkey does too, but is changed upstream]
#this only works if each system has data to compare
#have to change in 02_fit_prediction_model also, so that the correction gets modelled separately
  ...
  data$source[data$ihme_loc_id == "KOR" & data$vr == 1 & data$year > 1999] <- "VR1" #(doing this for 1970 doesn't actually help at all, which is why those are outliered)
...
############################################
  ...
  #Exception to some ZAF provinces . Set the VR to biased because the variance seems to be too small. 
  # The reason why VR is deemed complete has to do with the fact that VR captures the upward trend in HIV and other survey sources didn’t (9/2015)
  ...
  ###################################################
  #Everything that is "vr_biased" before here gets corrected upwards
  ...
  # For SRS countries (PAK, BGD,IND) and China and India subnational we add additional data variance (not HKG & MAC)
  ...
  # IND VR that actually needs correction
...
#####DEBUGGING
#gbg_vect <- unique(data$ihme_loc_id[data$category=="vr_biased"])
