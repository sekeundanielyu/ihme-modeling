...
# os.chdir('' % ('' if os.name=='posix' else ''))
# data = pl.csv2rec('prediction_model_results_all_stages.txt', missing='NA')

# # import data for test ihme_loc_id
# index = (data['ihme_loc_id'] == cc) & (data['data'] == 1)
# year = pl.array(data['year'][index])
# log10_mort = pl.array(data['log10_mort'][index])
# log10_var = pl.array(data['log10_var'][index])
# category = pl.array(data['category'][index])
# ##next two vars no longer needed for testing
# ##bias_vr = pl.array(data['biascoef'][index])
# ##avgbias = pl.array(data['regional_vrbias'][index])

# # import prior for test ihme_loc_id
# index = (data['ihme_loc_id'] == cc)
# pyear = pl.array(data['year'][index])
# pmort = pl.log(pl.array(data['pred2final'][index]))/pl.log(10) # this is to convert the prior to log base 10
