...
if (Sys.info()[1] == "Linux"){
 ...
 source(paste0(code_dir,"get_locations.r"))
}else{
 root <- ""
 hivsims <- F
 user <- ""
...
######################
 ...
 ## GBD 2015 changes
  ...
  # add DHS and MICS SBH as reference in select KEN subnats - 3/19/16
  ...
  # Kenya subnational, don't use the really jumpy DHS surveys as reference sources (mostly 2003 and 2008 DHS surveys)
  ...
  # for ZAF_483, add census HH points as part of reference (03/31/16)
  data$reference[data$ihme_loc_id=="ZAF_483" &  data$source.type=="Census_HH"] <- 1   
  
  # for ZAF_484, add census HH points as part of reference (03/31/16)
  ...
  data$reference[grepl("SAU", data$ihme_loc_id) & data$source=="VR"] <- 1
  ...
  data$reference[data$ihme_loc_id=="ZAF_488"] <- 0 # resetreference 1/4/16
  data$reference[data$ihme_loc_id=="ZAF_488" & data$source1=="43146#ZAF_IPUMS_CENSUS_1996 1996 indirect"] <- 1 # set 1996 census as reference 1/4/16
...
if(!hivsims){
  save(model, file="first_stage_regressions.rdata")
  save(model, file=paste("first_stage_regression_", Sys.Date() ,".rdata", sep=""))
}
...
#this grabbed the standard error, but I haven't figured out where this is in the nlme model yet
#src.re$re2.se <- se.ranef(model)$source1
...
write.csv(st.fe, paste0(root,"/WORK/02_mortality/03_models/2_5q0/diagnostics/source_type_fe_", Sys.Date(), ".csv"), row.names = T)
...
locs <- get_locations()
locs <- locs[,c("ihme_loc_id","level","parent_id","location_id","level_1", "level_2", "level_3")]

# 3 layers to include distinction between India states and India states + urban/rural
level_2 <- locs[locs$level_2 == 1 & !(locs$ihme_loc_id %in% c("CHN_354","CHN_361")),]
level_3 <- locs[locs$level_3 == 1 & !(locs$ihme_loc_id %in% c("CHN_354","CHN_361")),]
parents <- locs[locs$level_1 == 1,]

## determine which locations to keep after each level of space time
keep_level_2 <- unique(locs$ihme_loc_id[locs$level_1 == 0 & locs$level_2 == 1 & locs$level_3 == 0])
keep_level_3 <- unique(locs$ihme_loc_id[locs$level_1 == 0 & locs$level_3 == 1])
keep_parents <- unique(locs$ihme_loc_id[!(locs$ihme_loc_id %in% keep_level_2) & !(locs$ihme_loc_id %in% keep_level_3)])
  
# fit space-time for national and two subnational layers
  reg.level2 <- unique(data$region_name[data$ihme_loc_id %in% unique(level_2$ihme_loc_id)])  
  preds.level2 <- resid_space_time(stdata[stdata$region_name %in% reg.level2 & (stdata$ihme_loc_id %in% unique(level_2$ihme_loc_id)),])
  preds.level2 <- preds.level2[preds.level2$ihme_loc_id %in% keep_level_2,]

  reg.level3 <- unique(data$region_name[data$ihme_loc_id %in% unique(level_3$ihme_loc_id)])  
  preds.level3 <- resid_space_time(stdata[stdata$region_name %in% reg.level3 & (stdata$ihme_loc_id %in% unique(level_3$ihme_loc_id)),])
  preds.level3 <- preds.level3[preds.level3$ihme_loc_id %in% keep_level_3,]
  
  preds.nat <- resid_space_time(stdata[stdata$ihme_loc_id %in% unique(parents$ihme_loc_id),])
  preds.nat <- preds.nat[preds.nat$ihme_loc_id %in% keep_parents,]
  
  preds <- rbind(preds.level2,preds.level3,preds.nat)
...
write.csv(data, ifelse(!hivsims,
                         "prediction_model_results_all_stages.txt",
                         paste("prediction_model_results_all_stages_",rnum,".txt",sep = "")),row.names=F)

if(!hivsims) write.csv(data, paste("prediction_model_results_all_stages_", Sys.Date(), ".txt", sep=""),row.names=F)