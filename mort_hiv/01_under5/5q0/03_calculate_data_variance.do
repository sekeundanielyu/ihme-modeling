...
** bring in 5q0 database (vr data has already been adjusted for completeness)
	...
	else insheet using "prediction_model_results_all_stages_GBD2013.txt", clear
...
** CHN DSP: correct population using a dataset that 
	preserve 
...
** China province DSP population
	** bring in dataset
	...
	import delimited using "GBD_2013_locations.txt", clear
	rename local_id_2013 iso3
	keep iso3 ihme_loc_id
	replace iso3 = "CHN" if ihme_loc_id == "CHN_44533"
	drop if iso3 == ""
...
** calculate data variance for 5m0
	...
	** ugh bad hack i'm sorry this is horrible changeme- Chelsea
...
**************************************************************************************************
	** for China national and provinces, use birth numbers and the binomial distribution to calculate the variance in qx space
	...
	use "CHN_MCHS_1996_2012_BIRTHS_DEATHS_Y2013M10D08.DTA", clear
...
*******************************
   ...
   ** now I need to replace the stderrors for these points which are now including vr bias variance
...
** save final prepped file for GPR process
	...
	else outsheet super_region_name region_name ihme_loc_id year ldi_id maternal_educ hiv data category vr mort mort2 logit_mort mse pred1b resid pred2resid pred2final ptid source1 reference log10sdq5 biasvar logit_var using "gpr_5q0_input_GBD2013.txt", comma replace

	** save an archived version for reference
	if (!$hivsims) outsheet super_region_name region_name ihme_loc_id year ldi_id maternal_educ hiv data category vr mort mort2 logit_mort mse pred1b resid pred2resid pred2final ptid source1 reference log10sdq5 biasvar logit_var using "gpr_5q0_input_GBD2013_`c(current_date)'.txt", comma replace 
