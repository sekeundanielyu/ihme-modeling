...
** bring in 5q0 database (vr data has already been adjusted for completeness)
	else insheet using "first_stage_results_new.txt", clear
...
** add in papfam in as a source
	replace category = "papfam" if source == "PAPFAM" | source == "PAPCHILD"
...
** save final prepped file for GPR process
	if ($hivsims) outsheet super_region_name region_name ihme_loc_id year ldi_id maternal_educ hiv data category vr mort mort2 logit_mort pred1b resid  ptid source1 reference log10sdq5 biasvar logit_var using "gpr_5q0_input_$rnum.txt", comma replace 
	else outsheet super_region_name region_name ihme_loc_id year ldi_id maternal_educ hiv data category vr mort mort2 logit_mort pred1b resid  ptid source1 reference log10sdq5 biasvar logit_var using "gpr_5q0_input_new.txt", comma replace

	** save an archived version for reference
	if (!$hivsims) outsheet super_region_name region_name ihme_loc_id year ldi_id maternal_educ hiv data category vr mort mort2 logit_mort pred1b resid  ptid source1 reference log10sdq5 biasvar logit_var using "gpr_5q0_input_new_`c(current_date)'.txt", comma replace 
