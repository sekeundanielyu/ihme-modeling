...
## load data
  ...
  setwd(paste(ifelse(Sys.info()[1]=="Windows", "", ""), "", sep=""))

  
  stata <- read.dta("bin_res_for_smoothing.dta", convert.underscore=F, convert.factors=F)
...
## output to read into stata when actually apply model
  write.dta(fitted, file="bin_res_smoothed.dta")
