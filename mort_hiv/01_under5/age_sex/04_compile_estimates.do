...
** Description: compiles all countries' final estimates of early nn, late nn, 
**			  nn, post-nn, child, and under-5 mortality by sex 
** ***********************************************************
	...
	log using "compile_log.log", replace
	
** save country codes for later use
	...
	** find all country files; append together
	tempfile all 
	local missing ""
	local count = 0
	foreach ihme_loc_id of local all_locs { 
		di in red "`ihme_loc_id'"
		capture use "`ihme_loc_id'_noshocks.dta", clear 
		if (_rc != 0) {
	...
	** save list of missing files, break if there are some missing
	clear
	...
	drop row
	outsheet using "`out_dir'/diagnostics/missing_results.csv", comma replace
	...
	** Save estimates and save rates of decline
	...
	drop change* 
		...
		saveold "`out_dir'/results/estimated_enn-lnn-pnn-ch-u5_noshocks.dta", replace 
		saveold "`out_dir'/results/archive/estimated_enn-lnn-pnn-ch-u5_noshocks_`date'.dta", replace 
	...
	drop q*
		...
		saveold "`out_dir'/results/estimated_enn-lnn-pnn-ch-u5_ratesofchange_noshocks.dta", replace
		saveold "`out_dir'/results/archive/estimated_enn-lnn-pnn-ch-u5_ratesofchange_noshocks_`date'.dta", replace
	restore
	...
	** read in most recent older file to get comparison
	local files : dir "estimated_enn-lnn-pnn-ch-u5_noshocks*.dta"
	...
	saveold "comparison_runs.dta", replace
