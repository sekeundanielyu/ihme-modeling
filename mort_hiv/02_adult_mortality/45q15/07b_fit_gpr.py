...
# user = sys.argv[1]
os.chdir('strPath')
# How to get relational filepath with paste?
# os.chdir('strPath')
...
for iter in iters:
	for ss in sexes:
		if (huncert == int(1)):
			...
			os.chdir('strPath')
			data = pl.csv2rec('prediction_model_results_all_stages%s.txt' % (iter), missing='NA')
		else:
		...
		if (huncert == int(1)):
			...
			if (hitercount == 50):
				os.chdir('strPath')
				drawlist = pl.csv2rec('%s_chosen_draws.csv' % (cc), missing='NA')
				...
				os.chdir('strPath')
				pl.rec2csv(all_sim_comp, 'gpr_%s_sim_not_scaled%s.txt' %(cc, hsim))
			elif (hitercount == 1):
			...
			os.chdir('strPath')
			pl.rec2csv(all_sim, 'gpr_%s_%s_sim_not_scaled.txt' %(cc, ss))
	#end
