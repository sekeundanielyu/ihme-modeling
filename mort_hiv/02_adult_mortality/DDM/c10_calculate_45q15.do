...
** Filepaths  
** **********************
	if (c(os)=="Unix") global root "strPath"
	if (c(os)=="Windows") global root "strPath"
	global smoothed_comp_file = "strPath/d08_smoothed_completeness.dta"
	global pop_file = "strPath/d09_denominators.dta"
	global deaths_file = "strPath/d01_formatted_deaths.dta"
	global save_file = "strPath/d10_45q15.dta"
...
** get smoothed adult completeness estimates
	...
	** calculate usable completeness estimate 
	replace comp = pred
	replace comp = trunc_pred if regexm(source_type, "VR")==1 | regexm(source_type, "SRS")==1 | regexm(source_type, "DSP")==1
	replace sd = trunc_sd if regexm(source_type, "VR")==1 | regexm(source_type, "SRS")==1 | regexm(source_type, "DSP")==1
	...
	** duplicate for both sexes everywhere but SAU (SAU has estimates for both males and females due to differential completeness) 
	gen id = _n 
	...
	** we need to duplicate SSPC-DC so that it matches with both SSPC and DC 
	...
	// For all recombinations, we drop the years before/after the split where we didn't have data in the VR
	// For example, we would drop if source == "VR_pre2002" & year > 2002 since those estimates are covered by "VR_post2002"
	** recombine Turkey 
	drop if source == "VR1" & ihme_loc_id == "TUR" & year >= 2009 
	drop if source == "VR2" & ihme_loc_id == "TUR" & year < 2009
	replace source = "VR" if regexm(source, "VR") & ihme_loc_id == "TUR"
	
    ** recombine South Korea 
	drop if source == "VR1" & ihme_loc_id == "KOR" & year > 1977 
	drop if source == "VR2" & ihme_loc_id == "KOR" & year <= 1977
	...
	** Let's see if the Iran data pre-2003 (29/30 provinces) is different from post-2003 (urban cemetary data)
	drop if source == "IRN_VR_pre2003" & ihme_loc_id == "IRN" & year > 2003
	drop if source == "IRN_VR_post2003" & ihme_loc_id == "IRN" & year <= 2003	
	replace source = "VR" if inlist(source,"IRN_VR_pre2003","IRN_VR_post2003") & ihme_loc_id == "IRN"
...
** **********************	
	...
	replace source_type = "VR" if regexm(source_type,"CENSUS") & regexm(ihme_loc_id,"USA_") // temp fix to allow true US populations to be pulled through adequately
...
** Pull in sources where all we have is unadjusted mortality rates (these are select years from systems that we are able to DDM parts of)
	preserve
	use "strPath/BGD_SRS_1999_2005_MR_UNADJUSTED.DTA", clear
	drop if year == 2005
	append using "strPath/IND_SRS_1990_1991_1995_MR_UNADJUSTED.DTA"
	append using "strPath/IND_SRS_1981_2009_MR_UNADJUSTED.DTA"
	append using "strPath/USABLE_DZA_VR_2010_2011_MR.DTA"
...
** determine which countries are totally complete 
	...
	replace sd = 0 if min_comp > 0.95 & min_comp != . & inlist(source_type, "VR", "VR-SSA", "SRS", "DSP") & ihme_loc_id != "JAM" 
	replace adjust = 3 if min_comp > 0.95 & min_comp != . & inlist(source_type, "VR", "VR-SSA", "SRS", "DSP") & ihme_loc_id != "JAM"
	drop min_comp
	
// Use national-level completeness for subnationals if the national is fully complete
	preserve
	keep ihme_loc_id source_type adjust
	duplicates drop
	rename ihme_loc_id parent_loc_id	
	rename adjust parent_adjust
	keep if parent_adjust == 3
...
** make a manual exception for Taiwan and Hong Kong 
	replace comp = 1 if inlist(ihme_loc_id, "TWN", "CHN_354")
	replace sd = 0 if inlist(ihme_loc_id, "TWN", "CHN_354")
	replace adjust = 3 if inlist(ihme_loc_id, "TWN", "CHN_354")
...
** We say that if completeness is over .95 for at least 25 years past 1980, and it's on the CoD cause-specific VR list, it should be called good quality
	...
	** using cluster j path, open vr file sent to mortality
		use "strPath/VR_data_master_file.dta", clear
		** keep only the country years
		keep iso3 year
		duplicates drop
		** get rid of India (unless something changes in the future)
		drop if iso3=="IND"
		** done, save wherever
		tempfile cod_years
		save `cod_years'
	...
	// Get points marked as outliers in 45q15 
		insheet using "strPath/raw.45q15.txt", comma clear
    ...
    outsheet using "strPath/cod_completeness.csv", comma replace
    ...
    outsheet using "strPath/cod_comp_collapsed.csv", comma replace
