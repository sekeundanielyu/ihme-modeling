...
if (_N > 0) { 
	...
	if("`varname'" == "pop") {
		*******************drop duplicates***************************** 
		...
		duplicates drop ihme_loc_id year sex source_type month day pop*, force

		sort meandiff

		duplicates drop ihme_loc_id year sex source_type month day pop_source, force

		duplicates drop ihme_loc_id year sex source_type month day agegroup* pop0-pop100, force

		duplicates tag ihme_loc_id year sex source_type month day, g(dup)
		drop if pop_source == "IPUMS" & dup == 1
		drop dup

		sort meandiff
		
		capture: duplicates drop ihme_loc_id year sex source_type month day if ihme_loc_id ~= "CHN", force
