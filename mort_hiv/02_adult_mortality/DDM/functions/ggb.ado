...
// This is supposed to represent the number of entries (birthdays aged a) into the cohort at a given year
// Geometric mean of the population divided by the difference in ages between the age groups
...
forvalues i = 1/`maxnum' {
	// Try to figure out, for each census pair, what age group you should use given the interval that exists between censuses
	// We want to carry on each cohort through the entire census gap to get the average, if possible.
	// So if there's a twenty-year interval, we want age 30 estimate to represent the average of 10-30 in C1 and 30-50 in C2
	...
	qui {
		...
		forvalues j = 0/`iminus' {
			// If time is the limiting factor, then just go until you hit the age spacing where it is closest to time
			...
			// Idea is to match the spacing that of the openend - agegroup, but with the direction of the forvalues it becomes more difficult than ideal
