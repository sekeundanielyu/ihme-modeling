...
** **********************
	...
	global save_file "strPath/d00_compiled_population.dta"
...
** DYB 
	...
	** drop data that is not representative or missing data
	...
	** Make 1982 and 1990 Census populations reflect that those numbers are representative of 1981 and 1989
...
** HMD (GBR (and England/Wales, N Ireland, and Scotland), NLD, CHE, TWN, BEL - only census years that we want to use, except for subnational where we use all years)
    ...
    ** drop N Ireland and Scotland post 1980 because we will use data from Adrian Davis for these instead
    drop if CENSUS_SOURCE == "HMD" & inlist(COUNTRY,"XNI","XSC") & YEAR >= 1981
...
** **********************
** Albania. Taken out because it's not actually an Albanian census
	** append using "$rawdata_dir/ALB_CENSUS_2011/USABLE_ALB_CENSUS_2011.dta"
...
** Indonesia
	...
	** Not usable -- overstates deaths
	// append using "$rawdata_dir/SUSENAS/USABLE_SURVEY_SUSENAS_IDN_1996-1998_vPOPULATION.dta"
...
** Japan 
	...
	** only use these to fill in above sources
	duplicates tag CO YEAR SEX, gen(d)
	drop if d == 1 & CENSUS_SOURCE == "KENJI_SHIBUYA"
	drop d 
...
** ZAF: 2011 Census (Stats South Africa) more appropriate than the DYB 2012 numbers, use these instead
    append using "$newdata_dir/zaf_census/data/USABLE_ALL_AGE_POP_ZAF_CENSUS_SSA_2011.dta" 
	
** CAN 2012 CANSIM Census (This is the same as the numbers from the DYB 2012 update, so don't add it.)
	** append using "$rawdata_dir/CAN_VR/USABLE_CAN_2012_VR.dta"
...
** China 
	...
	** the following code substitutes provincial DSP estimates aggregated to national (must comment out the above 2 lines as well)
	...
	drop if !inlist(YEAR,1991,1992,1993,1994,1995,2001,2002,2012) & CENSUS_SOURCE == "CHN_DSP"
	replace source_type = "DSP" if source_type == "" 
	
	append using "$rawdata_dir/CHN_SSPC/USABLE_SSPC_CHN_1986-2008.dta"
	append using "$newdata_dir/chn_sspc_2010_2012/data/USABLE_ALL_AGE_POP_CHN_SSPC_2010_2012.dta" // JM 02.01.16
	replace source_type = "SSPC" if source_type == "" 
	
	append using "$rawdata_dir/CHINA 1 percent/USABLE_INT_GOV_CHN_1995_vPop.dta"
	// append using "$rawdata_dir/CHINA 1 percent/USABLE_INT_GOV_CHN_2005_vPop.dta" // Now brought in with subnational data
...
** TUR: 1989 Demographic Survey, survey population scaled up
...
** TUR: add in 2010, 2011, 2012 population from the registry system
	...
	replace source_type = "VR2" if source_type == "" & CENSUS_SOURCE == "TurkStat_ADNKS"
...
** Great Britain : England and Wales by Region
...
** IND urban rural
	...
	** DLHS III
		** append using "$newdata_dir/ind_dlhs3_urban_rural/data/USABLE_ALL_AGE_POP_IND_URBAN_RURAL_DLHS3.dta"
		** replace source_type = "DLHS3" if source_type == ""
...
** US Census subnational populations
...
** American Samoa population
	// Don't use because DYB already has census at slightly more granular data levels
	// append using "$newdata_dir/asm_census/data/USABLE_ALL_AGE_POP_ASM_1950_2000_CENSUS.dta"
...
** Ethiopia Census 1984: Excluded, Addis Ababa only
	// append using "$hhdata_dir/ETH_census_1984/data/USABLE_ALL_AGE_POP_ETH_CENSUS_1984.dta"
...
** DHS surveys, ERI 1995-1996 and 2002 NGA 2013, MWI 2010, ZMB 2007, RWA 2005, IND 1998-1999, DOM 2013, UGA 2006, HTI 2005-2006, ZWE 2005-2006, NIC 2001, BGD SP 2001, JOR 1990, DOM 2013
	...
	// append using "$hhdata_dir/IND_DHS_1998_1999/data/USABLE_ALL_AGE_POP_IND_DHS_1998_1999.dta" // Excluded: sample sample size + weird weights => small deaths numbers
	...
	append using "$hhdata_dir/DOM_SP_DHS_2013/data/USABLE_ALL_AGE_POP_DOM_SP_DHS_2013.dta"
	replace source_type = "HOUSEHOLD_SP_DHS" if source_type == "" // Don't want to analyze this in the same tranche as DOM_DHS_2013
	*/
		replace source_type = "HOUSEHOLD" if source_type == ""	
...
** Subnationals not previously run at the national level
	** IND SAGE
	// append using "$hhdata_dir/IND_SAGE_2007/data/USABLE_ALL_AGE_POP_IND_SAGE_2007.dta" 
	...
	** IND DLHS 2002-2005
	// append using "$hhdata_dir/IND_DLHS_2002/data/USABLE_ALL_AGE_POP_IND_DLHS_2002.dta" 
	
	** IND DLHS 2007-2008
	// append using "$hhdata_dir/ind_DLHS_2007/data/USABLE_ALL_AGE_POP_IND_DLHS_2007_2008.dta" 
	// replace source_type = "HOUSEHOLD_DLHS" if source_type == "" // Want to have this have a separate source_type otherwise it'll conflict with India SAGE data.
	
	** IND HDS
	// append using "$hhdata_dir/IND_HDS_2004_2005/data/USABLE_ALL_AGE_POP_IND_HDS_2004_2005.dta" 
	...
	** IND MCSS
	// append using "$hhdata_dir/ind_MCSS_2000_2000/data/USABLE_ALL_AGE_POP_IND_DHSL_2000_2001.dta" 
...
** Subnationals previously run at the national level 
	...
	** SAU Census 2004
	// append using "$hhdata_dir/SAU_CENSUS_2004/data/USABLE_ALL_AGE_POP_SAU_CENSUS_2004.dta" 
	
	** SAU DRB 2007
	// append using "$hhdata_dir/SAU_DRB_2007/data/USABLE_ALL_AGE_POP_SAU_DRB_2007.dta" 
...
** Drop CHN national data from provincial level estimates, as well as DYB 2010 census
...
** We want to use national populations for the Pakistan & Bangladesh SRS 
	preserve
...
** Relabel the 2000 census survey as a SURVEY since it duplicates years with SUSENAS
	replace source_type = "SUSENAS" if strpos(CENSUS_SOURCE,"SUSENAS") != 0 & CO == "IDN"
	replace source_type = "SUPAS" if strpos(CENSUS_SOURCE,"SUPAS") != 0 & CO == "IDN"
...
** **********************
	...
	replace pop_nid = "" if pop_nid == "." | pop_nid == " " // For some reason, it carries over the missing values as periods
