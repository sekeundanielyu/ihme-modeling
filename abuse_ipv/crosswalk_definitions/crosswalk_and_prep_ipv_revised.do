...
// Make locals for relevant files and folders 
	** Individual-level microdata from the WHO Multi-country Study on Women's Health and Domestic Violence against Women 
	local eg_cw_data				"$j/WORK/05_risk/risks/abuse_ipv_exp/data/exp/01_tabulate/raw/VAW extract GBD 120504.dta"
	** Age group id converstion 
	local age_groups				"$j/WORK/05_risk/risks/abuse_ipv_exp/data/exp/02_compile"
	** Ever been partnered DisMod output 
	local ever_partnered			"$j/WORK/05_risk/risks/abuse_ipv_exp/data/exp/03_adjust/ever_partnered/ever_partnered_draws.dta"
	** Log directory for crosswalk regression
	local logs						"$j/WORK/05_risk/risks/abuse_ipv_exp/data/exp/03_adjust/logs"
	** Directory for compiled tabulated data
	local data_dir 					"$j/WORK/05_risk/risks/abuse_ipv_exp/data/exp/02_compile"
	** Directory for files to be uploaded to Dismod
	local outdir 					"$j/WORK/05_risk/risks/abuse_ipv_exp/data/exp/2452/input_data"
	** Turn on/off crosswalk block (1="on", 0="off")
	local crosswalk					1


//** Ever had sex time series generated by Dismod model for GBD 2010
	//local had_sex_data				"$j/Project/GBD/RISK_FACTORS/Data/child_intimate_partner_abuse/data/ever_had_sex_30Mar2012.csv"
	//local ever_relationship_data	"$j/WORK/04_epi/01_database/01_code/02_central/02_temp/relationship_tab.dta"
...
// Load population data
	...
	// For recent years that we do not have proportion that have ever been in a relationship, we will assume it is the same as it was in the most recent year 
		//sort age location_id year
		//bysort age location_id: carryforward mean, replace
		//bysort age location_id: carryforward draw_*, replace
	...
	//save "$j/WORK/2013/05_risk/02_models/abuse_ipv/temp/eversexfinal_revised.dta", replace
...
/* If crosswalk block was turned off	
	else {
...
// Adjust for fraction who have had sex
	...
	// Create 1,000 draws based on mean and standard error to multiply by 1,000 draws from DisMod model 
		...
		// drawnorm function to create uncertainty around raw data points 
		 //forvalues draw = 0/999 {
			//gen draw_`draw' = rnormal(mean, standard_deviation)
		//}
...
// Make data status and study status variables consistent
	//gen study_status = "active" if !inlist(data_status, "excluded", "outlier")

	//drop if data_status == "excluded"\
	//replace data_status = "outlier" if data_status == "excluded" // don't want to drop entirely, just want to outlier 

	// replace is_outlier
	//replace is_outlier = 1 if data_status == "outlier"
...
// Replace uncertainty type with effective sample size (ESS) if collapsing was done across age groups 
	...
	replace standard_error = . if flag != 0 // these standard errors haven't been aggregated; using ESS instead 
	replace uncertainty_type_value = . if flag != 0 
	replace effective_sample_size = . if effective_sample_size == 0 // was missing before and was replaced with 0 in the sum 
	replace standard_error = . if standard_error == 0 // was missing before and was replaced with 0 in the sum 
...
// Save clean and crosswalked dataset 
	export excel "$j/WORK/05_risk/risks/abuse_ipv_exp/data/exp/03_adjust/compiled_adjusted_tabulations_full_dataset.xlsx", sheet("Data") firstrow(variables) sheetreplace
...
// Just add new studies
	...
	export excel "$j/WORK/05_risk/risks/abuse_ipv_exp/data/exp/2452/input_data/gbd2015_data_abuse_ipv_$S_DATE.xlsx", firstrow(variables) sheet("extraction") replace
...
/* 
	...
	// import updated data and append on new data
	local dismod_dir "$j/WORK/05_risk/risks/abuse_ipv_exp/data/exp/2452/review/upload" 
	import excel "`dismod_dir'/me_2452_ts_2015_12_31_re_upload_01072016.xlsx", firstrow clear

	append using `new_data', force

	foreach var in sex_issue measure_issue age_issue year_issue { 
		replace `var' = 0 if `var' == . 
	}

	drop file iso3
	
	// Just uploaded new data addition 
	
	// 2/17/16 
	//keep if nid == 239733

	// 2/9/2016 Added literature sources for IPV
	//keep if regexm(note_modeler, "Literature") 


	//keep if iso3 == "JAM" & year_start == 1997
