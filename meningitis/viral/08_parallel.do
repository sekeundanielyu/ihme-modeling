...
// WRITE CODE HERE
	// get draws
	// this could easily be made to include incidence
	get_draws, gbd_id_field(modelable_entity_id) gbd_id(1296) measure_ids(5) location_ids(`location') status(best) source(epi) clear
	drop if age_group >= 22

	foreach year of local years {
		foreach sex of local sexes {
			preserve
			keep if year_id == `year' & sex_id == `sex'
			merge m:1 sex_id age_group_id using "`in_dir'/bac_vir_ratio_2015.dta", keep(3) nogen
			forvalues a = 0/999 {
