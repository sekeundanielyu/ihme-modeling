...
# set options
prediction.method <- "spline" #suggested for GBD2015
...
gridded.dataset.dir <- file.path(j_root, "WORK/2013/05_risk/01_database/02_data/air_pm/01_exp/02_nonlit/02_inputs/05_other/gridded_dataset")
gridded.dataset <- file.path(gridded.dataset.dir, "GBD2013_2014_05_30.csv") # GBD2013 final version

###Output### 	
# where to output the split gridded files
out.dir <-  file.path("/share/gbd/WORK/05_risk/02_models/02_results/air_ozone/exp/gridded", version)
...
setkeyv(pollution, c("ihme_loc_id", "x", "y"))
...
global.list <- mclapply(na.omit(unique(pollution$ihme_loc_id)),
                        castAndSave,
                        global.dt = pollution,
