...
if (Sys.info()["sysname"] == "Windows") {
df <- fread(paste0(j_root, "WORK/05_risk/risks/envir_radon/data/exp/prepped/data.csv"))
}
#********************************************************************************************************************************
...
# Ideas: test that variance=raw_variance / data=raw_data in cases where the covariates are already gold standard
# Also create a metadata ID for each datapoint such that you can split off the metadata and run through ST-GPR, then merge it back on to results for graphing
...
#----MAKE PREDICTIONS-------------------------------------------------------------------------------------------------------------
if (Sys.info()["sysname"] == "Windows") { # only do this for testing purposes if running locally, otherwise prior model is in another script
...
## Linear model
# Run model again without the study level covariates that you previously crosswalked
mod <- lmer(data ~ subnational + (1|location_id) + (1|region_id), 
            data=df, 
            na.action=na.omit)

# Now predict using the model object
df$prior2 <- predict(mod, newdata=df, re.form = NA, allow.new.levels=TRUE)
}
#********************************************************************************************************************************
