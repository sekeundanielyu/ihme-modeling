...
<p>{model_code}</p>
<p>Please direct any questions that you have to {TechSupport} ({TechEmail}).</p>
<p></p>
<p>CODEm2.0</p>
'''

if __name__ == "__main__":
    user, model_version_id = sys.argv[1:]
    acause = get_acause(model_version_id)
    model_code = "/ihme/codem/data/{0}/{1}".format(acause, model_version_id)
    busted_code = "/ihme/codem/code/{0}".format(model_version_id)
    ...
    if os.path.exists(busted_code):
        ...
        s = Server("strConnection", 'strUser', 'strPassword')
        s.connect()
        e = Emailer(s)
        e.add_recipient(recipient)
        e.add_attachment(error_file)
        e.set_subject('CODEm run failure')
        e.set_body(frame_work.format(user=user, model_code=model_code,
                                     model_version_id=model_version_id, acause=acause))
        e.send_email()
        s.disconnect()
