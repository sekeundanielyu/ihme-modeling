...
def calculate_lamda_array(full, train, lambda_time_smooth, lambda_time_smooth_nodata):
    '''
    (data frame, data frame, float, float) -> array
...
def timeW(full_sub, train_sub, omega_age_smooth, lambda_time_smooth, lambda_time_smooth_nodata):
    '''
    ...
    aMax = np.maximum(EV("abs(i2-start)"), EV("abs(end-i2)")).astype("float32") # argMax vector
    return EV("(1/exp(omega_age_smooth*abs(ageS-i1))) * (1 - (abs(yearS-i2)/(aMax+1))**l)**3")
...
def spacetime(reg, sReg, df, ko, omega_age_smooth, lambda_time_smooth,
    ...
    Wat = timeW(full_sub, train_sub, omega_age_smooth,
                lambda_time_smooth, lambda_time_smooth_nodata).astype("float32")
