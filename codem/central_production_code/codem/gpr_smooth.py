...
def find_variance_type(df, ko):
    ...
    for location in locations:
        for age in age_groups:
            ...
            representative_count = sum((df2.source_type == "VR") &
...
def age_group_nsv(age_df, res_df, simple_df):
    ...
    sweet = age_df[(age_df.variance_type == "rich")
                   & (age_df[train_var])].index
    not_sweet = age_df[(age_df.variance_type != "rich") &
                       (age_df[train_var])].index
    intermediate = age_df[(age_df.variance_type == "intermediate") &
                   (age_df[train_var])].index
    sparse = age_df[(age_df.variance_type == "sparse") &
                  (age_df[train_var])].index
    amplitude = {"rich": mad(res_df.loc[sweet].as_matrix(), axis=0) * 1.4826,
                 "intermediate": mad(res_df.loc[not_sweet].as_matrix(), axis=0) * 1.4826}
    amplitude["sparse"] = amplitude["intermediate"]
    nsv = {"rich": mad(simple_df.loc[sweet].as_matrix(), axis=0) * 1.4826,
           "intermediate": mad(simple_df.loc[intermediate].as_matrix(), axis=0) * 1.4826,
           "sparse": mad(simple_df.loc[sparse].as_matrix(), axis=0) * 1.4826}
...
def median_amplitude(amp):
    ...
    if not all([check_amplitude_array_validity(amplitudes[a]["intermediate"]) for a in amplitudes.keys()]):
