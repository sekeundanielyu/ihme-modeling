rm(list=ls())
output_file <- file("covariate_selection.Rout", open="wt")
sink(output_file)
sink(output_file, type="message")

# get the neccassary libraries
.libPaths("/share/local/codem/R-3.1.2/library")
...
df <- fread ("input_database_square.csv", verbose=F)
...
pasteModel <- function(x, y){
    paste0("lmer(", y, " ~ ", x,
           ...
           ", data=df)")
           #"+ (0 + has_sub_nat |sub_nat_nest), data=df)")
}
...
mci_to_cmvi <- function(model_covariate_id){
    ...
    con <- dbConnect(MySQL(), user='strUser', password='strPassword', dbname="strConnection",
                     host='strConnection')
...
sink("cv_selected.txt"); cat(am_JSON); sink()
