...
class Model_List:
    ...
    def super_region_positions(self, reg, sReg, df, ko):
        '''
    ...
    def single_st(self, reg, sReg, df, ko, omega_age_smooth, lambda_time_smooth,
        ...
        '''
        pos = self.super_region_positions(reg, sReg, df, ko)
        st = ST.spacetime(reg, sReg, df, ko, omega_age_smooth, lambda_time_smooth,
                          lambda_time_smooth_nodata, zeta_space_smooth,
                          zeta_space_smooth_nodata)
        ...
        mini = df[(df.region == reg)][["pop", "envelope"]]
    ...
    def spacetime_predictions(self, df, ko, omega_age_smooth, lambda_time_smooth,
        ...
        regions = df[ko.ix[:,0]][["region", "super_region"]].drop_duplicates()
        regions.reset_index(inplace=True)
        for i in range(regions.shape[0]):
            self.single_st(regions["region"][i], regions["super_region"][i], df, ko, omega_age_smooth,
