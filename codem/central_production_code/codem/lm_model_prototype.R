rm(list=ls())
output_file <- file("linear_model_build.Rout", open="wt")
sink(output_file)
...
all_models <- fromJSON("cv_selected.txt")
...
sink("linear_model_json.txt"); cat(lm_json); sink()
sink("space_time_json.txt"); cat(space_json); sink()
