"""
Ensemble class that runs the model at the highest level and saves results
"""
...
class Ensemble:
    ...
    """
    def __init__(self, model_version_id, log_results=True):
        """
        Queries data base for model data and begins the model process
        ...
        """
        self.warnings = warn.WarningLog(model_version_id)
        if log_results:
            self.warnings.time_stamp("Querying for model parameters")
        self.mod_inputs = Inputs(model_version_id, update=log_results)
        self.log_results = log_results
        base_dir = "/ihme/codem/data/{0}/{1}"
        self.model_dir = base_dir.format(self.mod_inputs.acause, model_version_id)
        rSource = "/ihme/codem/code/{}/codem/prod/codem/".format(model_version_id)
        self.rSource = rSource
        if not os.path.exists(self.rSource):
            self.rSource = "/homes/strUser/codem/prod/codem/"
    ...
    def adjust_input_data(self, save=True):
        ...
        if save:
            pd.concat([self.mod_inputs.data_frame,
                      ...
                      axis=1).to_csv("input_database_square.csv", index=False)
            self.mod_inputs.priors.to_csv("priors.csv", index=False)
    ...
    def covariate_selection(self):
        ...
        subprocess.call("/share/local/codem/R-3.1.2/bin/Rscript " + self.rSource + "cvSelection.R " +
                        str(self.model_version_id), shell=True)
    ...
    def apply_gp_smoothing(self):
        """
        Apply gaussian process smoothing to space time results.
        """
        if self.log_results:
    ...
    def cleanup_code_base(self):
        ...
        subprocess.call("rm -rf /ihme/codem/code/{}".format(self.model_version_id), shell=True)
    ...
    def reload_data(self):
        ...
        self.mod_inputs.data_frame, self.mod_inputs.covariates, self.mod_inputs.priors = \
            Q.getCodemInputData(self.model_version_id)
        self.adjust_input_data(save=False)
        st_models_temp = \
            All_Models(pd.concat([self.mod_inputs.data_frame,
                                  self.mod_inputs.covariates], axis=1),
                       self.mod_inputs.knockouts,
                       self.mod_inputs.linear_floor_rate, json_dict, make_preds=False)
    ...
    def plot_location(self, location_id, age_group_id, model_space="death", submodels=None):
        ...
        sub_df = sub_df.query("location_id == {l} & age == {a}".format(l=location_id, a=age_group_id))
...
def load_codem_model(model_version_id):
    """
    ...
    """
    acause = Q.acause_from_id(model_version_id)
    f = "/ihme/codem/data/{acause}/{model}/codem_model.p".format(acause=acause, model=model_version_id)
