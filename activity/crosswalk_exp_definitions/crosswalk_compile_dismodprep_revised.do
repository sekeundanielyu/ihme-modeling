...
// Create locals for relevant files and folders
	local data_dir "$prefix/WORK/05_risk/risks/activity/data/exp/prepped"
	local files: dir "`data_dir'" files "*prepped.dta"
	local outdir "$prefix/WORK/05_risk/01_database/02_data/activity/01_exp/03_review/05_upload"
	local logs "$prefix/WORK/05_risk/risks/activity/data/exp/raw/logs" 
	local dismod_dir "$prefix/WORK/05_risk/risks/activity/data/exp"
...
// 1.) Append datasets for each extracted microdata survey series/country together 
		...
		foreach file of local files {
			if "`file'" != "whs_prepped.dta" & "`file'" != "tabulated_prepped.dta" {
				di in red "`file'" 
	...
	// Reshape long: Instead of having a mean activity variable for each activity threshold, will have 1 mean variable and one row per activity level
		...
		reshape long mean_@ se_@, i(file location_id survey_name iso3 year_start year_end sex age_start age_end) j(healthstate, string)
...
// 2.) Bring in reformatted/prepped tabulated data that was provided to us by experts/collaborators
	append using "`data_dir'/ensanut_formatted.dta"
	append using "`data_dir'/irn_surfncd_formatted.dta"
	
	tostring location_id, replace
	
// 3.) Bring in extracted tabulated data
	append using "`data_dir'/tabulated_prepped.dta"
...
// 4.) Prepare dataset for crosswalk 
	// Merge with country codes database to get parent ISO3 because the proportion urban covariate database does not have subnational estimates 
		// merge m:1 iso3 using `countrycodes', update keep(3 4 5) nogen
...
// 4a.) Run GPAQ-IPAQ regression for females and based on nationally representative data sources only, stratified by healthstate, with an interaction between super-region and GPAQ & controlling for age
		log using "`logs'/gpaq_regression_log.smcl", replace
			...
			foreach healthstate of local healthstates {
				...
				** Extract coefficients and covariance matrices for GPAQ, GPAQ-super region interaction 
				mat b = e(b)' // ' creates columnar matrix rather than default row matrix
				mat b2 = b[7..13,1]
				mat v = e(V)
				mat v2 = v[7..13,7..13]
		...
		save "J:/WORK/05_risk/risks/activity/data/rr/gpaq_ipaq_crosswalk_coefficients.dta", replace 
...
// 4b.) Run urbanicity regression stratified by healthstate, with an interaction between sex and urbanicity & controlling for age
		log using "`logs'/urbanicity_regression_log.smcl", replace
			foreach healthstate of local healthstates {
				di in red "Healthstate: `healthstate'"
...
// Set up to merge with GHDx database (based on file paths so need to parse file path) 
	...
	replace path = "J:/DATA/USA/NATIONAL_HEALTH_AND_NUTRITION_EXAMINATION_SURVEY/2007_2008" if path == "J:/DATA/USA/NATIONAL_HEALTH_AND_NUTRITION_EXAMINATION_SURVEY//2007_2008"
	replace path = "J:/DATA/USA/NATIONAL_HEALTH_AND_NUTRITION_EXAMINATION_SURVEY/2009_2010" if path == "J:/DATA/USA/NATIONAL_HEALTH_AND_NUTRITION_EXAMINATION_SURVEY//2009_2010"
	replace path = "J:/DATA/USA/NATIONAL_HEALTH_AND_NUTRITION_EXAMINATION_SURVEY/2011_2012" if path == "J:/DATA/USA/NATIONAL_HEALTH_AND_NUTRITION_EXAMINATION_SURVEY//2011_2012"
	...
	replace path = "J:/DATA/WHO_STEPS_NCD/BGD/2009_2010" if path == "J:/DATA/WHO_STEPS_NCD/BGD/2009"
	replace path = "J:/DATA/WHO_STEPS_NCD/BTN/THIMPHU_2007" if path == "J:/DATA/WHO_STEPS_NCD/BTN/2007" 
	replace path = "J:/DATA/WHO_STEPS_NCD/CIV/LAGUNES_2005" if path == "J:/DATA/WHO_STEPS_NCD/CIV/2005"
	replace path = "J:/DATA/WHO_STEPS_NCD/COD/KINSHASA_2005" if path == "J:/DATA/WHO_STEPS_NCD/COD/2005"
	
	replace path = "J:/DATA/ARG/NATIONAL_SURVEY_OF_RISK_FACTORS_ENFR/2005" if path == "J:/data/arg/national_survey_of_risk_factors_enfr/2009"
	replace path = "J:/DATA/WHO_STEPS_NCD/IRN/2007" if path == "" & iso3 == "IRN" & year_start == 2007
	replace path = "J:/DATA/WHO_STEPS_NCD/IRN/2011" if path == "" & iso3 == "IRN" & year_start == 2011
...
// Bring in full file 
	...
	// Replace any nids that are still missing 
		...
		local nsw "http://www.healthstats.nsw.gov.au/Indicator/beh_phys_age/beh_phys_age?filter1ValueId=&filter2ValueId"

		replace nid = 139725 if path == "`nsw'" & year_start == 2002
		replace nid = 139726 if path == "`nsw'" & year_start == 2003
		replace nid = 139728 if path == "`nsw'" & year_start == 2004
		replace nid = 139729 if path == "`nsw'" & year_start == 2005
		replace nid = 139730 if path == "`nsw'" & year_start == 2006
		replace nid = 139731 if path == "`nsw'" & year_start == 2007
		replace nid = 139732 if path == "`nsw'" & year_start == 2008 
		replace nid = 139733 if path == "`nsw'" & year_start == 2009 
		replace nid = 139734 if path == "`nsw'" & year_start == 2010 
		replace nid = 139735 if path == "`nsw'" & year_start == 2011
		replace nid = 139736 if path == "`nsw'" & year_start == 2012 
		
		// National Nutrition and Physical Activity Survey (NNPAS)
		
		replace nid = 113243 if path == "http://www.abs.gov.au/ausstats/abs@.nsf/Lookup/59FF6FE5D62BABE1CA257BAC00225A07?opendocument"
...
// Check sources with missing mean (COG was incorrectly extracted)
...
// Outsheet compiled dataset 
	outsheet using "J:/WORK/05_risk/risks/activity/data/exp/compiled_dataset.csv", comma names replace 

/*
// Just keep new data to add for now (5/30/16) - Brazil National Health Survey (PNS), also reassigning VIGITEL for Distrito Federal
...
// Just add BRFSS state level estimates that have been inflated using NHANES (6/3/2016)
...
// Physical inactivity and low physical activity, inactive (9356)
	...
	gen modelable_entity_name = "GBD 2015 Physical inactivity and low physical activity, inactive"
	...
	export excel "`dismod_dir'/9356/input_data/gbd2015_activity$S_DATE.xlsx", firstrow(variables) sheet("extraction") replace
	...
	gen modelable_entity_name = "GBD 2015 Physical inactivity and low physical activity, low active" 
	...
	export excel "`dismod_dir'/9357/input_data/gbd2015_activity$S_DATE.xlsx", firstrow(variables) sheet("extraction") replace
	...
	gen modelable_entity_name = "GBD 2015 Physical inactivity and low physical activity, moderately active" 
	...
	export excel "`dismod_dir'/9358/input_data/gbd2015_activity$S_DATE.xlsx", firstrow(variables) sheet("extraction") replace
	...
	gen modelable_entity_name = "GBD 2015 Physical inactivity and low physical activity, highly active " 
	...
	export excel "`dismod_dir'/9359/input_data/gbd2015_activity$S_DATE.xlsx", firstrow(variables) sheet("extraction") replace
	...
	gen modelable_entity_name = "GBD 2015 Physical inactivity and low physical activity, low/moderately/highly active" 
	...
	cap export excel "`dismod_dir'/9360/input_data/gbd2015_activity$S_DATE.xlsx", firstrow(variables) sheet("extraction") replace
	...
	gen modelable_entity_name = "GBD 2015 Physical inactivity and low physical activity, moderately/highly active" 
	...
	cap export excel "`dismod_dir'/9361/input_data/gbd2015_activity$S_DATE.xlsx", firstrow(variables) sheet("extraction") replace
