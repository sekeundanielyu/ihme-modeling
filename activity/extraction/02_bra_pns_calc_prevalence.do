...
// add logs
	//log using "/share/epi/risk/temp/activity/logs/log_`state'.smcl", replace 
	
	//di "`state'"

// Make locals for relevant files and folders
	local data_dir "$j/DATA/Incoming Data/BRA/NATIONAL_HEALTH_SURVEY_PNS/2013"
	local out_dir "$j/WORK/05_risk/risks/activity/data/exp"

// Bring in dataset 
	
	use "`data_dir'/BRA_PNS_2013_INDIV.DTA", clear 
...
// save for custom prep 
	save "$j/WORK/05_risk/risks/activity/data/exp/for_luisa/pns_compiled.dta", replace 
...
// Merge on state and region names 
	import excel "$j/WORK/05_risk/risks/activity/data/exp/raw/pns_codebook.xlsx", firstrow clear 
...
// Get stored prevalence calculations from matrix
		...
		getmata state age_start sex sample_size highactive_mean highactive_se modactive_mean modactive_se lowactive_mean lowactive_se inactive_mean inactive_se lowmodhighactive_mean lowmodhighactive_se modhighactive_mean modhighactive_se
...
// Save survey weighted prevalence estimates 
		save "/share/epi/risk/temp/activity/pns_`state'.dta", replace
