// DATE: July 21, 2015 (original date: DECEMBER 18, 2013)
...
// add logs
	log using "/share/epi/risk/temp/activity/brfss_state_extractions/logs/log_`us_state'.smcl", replace 
	
	di "`us_state'"

// Bring in compiled file 
	use "/snfs1/WORK/05_risk/risks/activity/data/exp/raw/brfss_clean_revised.dta", clear 
...
// Create empty matrix for storing proportion of a country/age/sex subpopulation in each physical activity category (inactive, moderately active and highly active)
	...
	// Get stored prevalence calculations from matrix
		clear
		getmata state year_start age_start sex sample_size highactive_mean highactive_se modactive_mean modactive_se lowactive_mean lowactive_se inactive_mean inactive_se lowmodhighactive_mean lowmodhighactive_se modhighactive_mean modhighactive_se
	...
	// Replace standard error as missing if its zero (so that dismod calculates error using the sample size instead)
		recode inactive_se lowactive_se lowmodhighactive_se modhighactive_se modactive_se highactive_se (0 = .)
	
	//  Organize
		order iso3 state year_start year_end sex age_start age_end sample_size highactive* modactive* inactive* lowmodhighactive* modhighactive*, first
		sort sex age_start age_end		
		
	// Save survey weighted prevalence estimates 
		save "/share/epi/risk/temp/activity/brfss_state_extractions/brfss_`us_state'.dta", replace
