...
// Create locals for relevant files and folders
	local data_dir = "J:/WORK/05_risk/risks/activity/data/exp/raw/brfss"
...
// Years 2011 and 2013 (most recent BRFSS) 	// can't find raw age variable (only imputed variable that they calculated); also is it comparable?
...
// Make 5-year age groups 
	...
	/*
// Apply ratios of NHANES:BRFSS to inflate BRFSS estimates, which are a known underestimate due to the fact that the questionnaire does not ask about occupational activity (which NHANES does)
	use "J:/WORK/05_risk/risks/activity/data/exp/ratio_brfss_nhanes.dta", clear 

	merge 1:m sex age_start using `compiled', keep(3) nogen

	// Apply ratio
	drop if total_mets == . 
	gen total_mets_adjusted = total_mets / ratio 

	// Only apply inflation if they answer that their work involves heavy labor 
	replace total_mets = total_mets_adjusted if workactive == 1 
	*/
	
// Save cleaned and compiled raw dataset	
	save "J:/WORK/05_risk/risks/activity/data/exp/raw/brfss_clean_revised.dta", replace
