...
// Standardize subnational unit names 

	replace region = strproper(region) 
	replace region = "East of England" if region == "East Of England" 
	replace region = "Yorkshire and the Humber" if region == "Yorkshire & The Humber" 
	replace region = "Yorkshire and the Humber" if region == "Yorkshire And The Humber" 
	replace region = "Yorkshire and the Humber" if region == "Yorkshire & The Humberside" 
	replace region = "North East England" if region == "North East"
	replace region = "North West England" if region == "North West" 
	replace region = "South East England" if region == "South East"
	replace region = "South West England" if region == "South West" 
	replace region = "Greater London" if region == "London" 
	replace region = "North West England" if region == "North West & Merseyside" 
	replace region = "East of England" if region == "Eastern" 
...
// Calculate proportion with work activity in each year/age/sex subgroup and save compiled/prepped dataset
	...
	// Also make subnational-level estimates 
	...
	foreach region of local regions {
		foreach year of local years {
				foreach sex in 1 2 {
					foreach age of local ages {
						count if region == "`region'" & year_start == `year' & age_start == `age' & sex == `sex'
						if `r(N)' > 0 {	
							di in red  "region: `region' year:`year' sex:`sex' age:`age'"
							mata: sample_size = sample_size \ `r(N)'
						// Calculate mean and standard error 
							** Years after 2000 have survey weights
							if `year' > 2000 {
								svy linearized, subpop(if region == "`region'" & year_start == `year' & age_start == `age' & sex == `sex'): mean workactive
									matrix meanmatrix = e(b)
									local mean = meanmatrix[1,1]
									mata: workactive_mean = workactive_mean \ `mean'
							...
							** Years before 2000 do not have survey weights
							if `year' < 2000 {
								mean workactive if region == "`region'" & year_start == `year' & age_start == `age' & sex == `sex'
									matrix meanmatrix = r(table)
									local mean = meanmatrix[1,1]
									mata: workactive_mean = workactive_mean \ `mean'
							...
							// Extract other key variables	
								mata: region = region \ "`region'" 
								mata: year_start = year_start \ `year'
								mata: sex = sex \ `sex'
								mata: age_start = age_start \ `age'
								levelsof file if year_start == `year' & age_start == `age' & sex == `sex' & region == "`region'", local(filepath) clean
								mata: file = file \ "`filepath'"
						}
					}
				}
	...
	// Get stored prevalence calculations from matrix
		clear
		getmata region year_start age_start sex sample_size file workactive_mean workactive_se
	
		drop if _n == 1 // drop top row which was a placeholder for the matrix created to store results
		
		append using `mata_calc' 
		
		merge m:1 region using `countrycodes' 
		drop if _m == 2 
		drop _m
