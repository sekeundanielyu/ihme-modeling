...
class Task(object):
    ...
    """
    def __init__(self, name, command, log_path, project_name, slots=10, retry_attempts=3, resume=False):
        # Basic task args
        self.name = name
    ...
    def submit_job(self, submit=True):
        ...
        if submit:
            if self.attempt < self.retry_attempts:
                ...
                print "Job {} has reached the maximum number of retries".format(self.name)
...
class TaskList(object):
    ...
    def add_task(self, task, dependencies):
        if task.name in self.tasks:
            raise KeyError("Task list already contains a job named {}".format(task.name))
        else:
            for d in dependencies:
                if d not in self.tasks:
                    raise KeyError("Dependency {} does not exist in the current task list".format(d))
            self.tasks[task.name] = {'task': task, 'dependencies': dependencies}
            self.not_started += 1
            self.all_jobs += 1
    ...
    def qstat(self):
        ...
        if len(jobs['job_info']['job_info']) > 0:
            for j in jobs['job_info']['job_info']['job_list']:
        ...
        try:
            ...
            for job_name in self.tasks:
                if resume:
                    ...
                    skip_statuses = ["Not started", "Completed", "Retry exceeded"]
                    default_status = "Failed"
                if self.tasks[job_name]['task'].status in skip_statuses:
                    pass
                else:
                    # Set all jobs that have not completed or started to Failed
                    self.tasks[job_name]['task'].status = default_status
                    self.tasks[job_name]['task'].cluster_status = None
                    # Check if job has been submitted to cluster or has a completed log file
                    if len(qstat.ix[qstat['name']==job_name]) > 0:
                        self.tasks[job_name]['task'].cluster_job_id = qstat.set_index('name').ix[job_name, 'job_id']
                        self.tasks[job_name]['task'].cluster_status = qstat.set_index('name').ix[job_name, 'state']
                        if self.tasks[job_name]['task'].cluster_status in ['r', 't', 'Rr', 'Rt']:
                            self.tasks[job_name]['task'].status = "Running"
                        elif self.tasks[job_name]['task'].cluster_status in ['qw', 's', 'S']:
                            self.tasks[job_name]['task'].status = "Submitted"
                    elif os.path.isfile(self.tasks[job_name]['task'].log_path):
                        if 'All done' in open(self.tasks[job_name]['task'].log_path).read():
    ...
    def run_jobs(self):
        for job_name in self.tasks:
            if self.tasks[job_name]['task'].status in ["Running", "Submitted", "Completed", "Retry exceeded"]:
                pass
