...
// Purpose:		Submit death draws for age-sex splitting
...
** **************************************************************************
	...
	** ****************************************************************
		...
		** ****************************************************************
			capture log close
			** log using "`log_folder'/03_agesex_split_`metric'_`today'.log", replace
	...
	// Split apart into smaller chunks by iso3 code to do the final formatting
		...
		foreach ihme_loc_id of local ihme_loc_ids {
			...
			if _rc {
				...
				!/usr/local/bin/SGE/bin/lx24-amd64/qsub -pe multi_slot 5 -l mem_free=10g -P "proj_custom_models" -N "COD_`acause'_`ihme_loc_id'_agesex_split" "`code_folder'/shellstata.sh" "`code_folder'/format_for_agesplit.do" "${username} `acause' `metric' `ihme_loc_id'"
			}
			restore
