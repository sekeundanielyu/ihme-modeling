** Description :(1)	Calculate population incidence rate = hazard *(1-prevalence) at the draw level	
*				(2) Calculate prevalence as prevalence= population incidence rate*duration
...
merge m:m location_id year age sex using `inc', keep(3) nogen
      ...
      forvalues i = 0/999 {
        ...
        quietly replace draw_`i' = draw_`i' * (7/365.242)
      }
	  drop if age >=90
	  drop prev*
...
save "/ihme/gbd/WORK/04_epi/01_database/02_data/`acause'/temp/draws/`acause'_prev_draws_`model_version_id'.dta", replace
...
foreach location_id of local ids {
    ...
    save_results, modelable_entity_id(1440) description(mild infection due to varicella `model_version_id') mark_best(yes) in_dir(/ihme/gbd/WORK/04_epi/01_database/02_data/`acause'/temp/draws/`model_version_id') metrics(prevalence)
