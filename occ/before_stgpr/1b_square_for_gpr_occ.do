/* Producing a square dataset for Exposures to Occupational Risks for occupations
12/18/15
*/
...
local outdata	"J:/WORK/05_risk/risks/occ/raw/occ_occ"
...
use "`rawdata'/logit_occupation_forspacetime_13Feb2015_1", clear
append using "`rawdata'/logit_occupation_forspacetime_13Feb2015_2.dta"
...
keep location_id survey natlrep ihme_loc_id year_id sex_id nid logit_exp_cat1-logit_exp_cat7
gen age_group_id = 22
...
replace categories="Administrative_managerial" if categories=="Administrative/managerial"
replace categories="Production_transport_laborers" if categories=="Production/transport/laborers"
replace categories="Professional_technical" if categories=="Professional/technical"

save "`outdata'/master_occ_occ.dta", replace
preserve
...
foreach category of local cats {
	...
	gen me_name = "occ_occ_"+lower("`category'")
	local location = "`outdata'/occ_occ_"+lower("`category'")+"_male.dta"
...
foreach category of local cats {
	...
	keep if categories == "`category'"
	gen me_name = "occ_occ_"+lower("`category'")
	local location = "`outdata'/occ_occ_"+lower("`category'")+"_female.dta"
	save "`location'", replace
}
