...
include "$j/WORK/10_gbd/00_library/functions/get_best_model_versions.ado"
include "$j/WORK/10_gbd/00_library/functions/get_estimates.ado"
...
// Get Hib vaccine coverage estimates
...
use "$j/temp/user/LRI/MidFiles/pneumo_vaccine_covs.dta", clear
save `vtype', replace

use "$j/temp/user/LRI/dismod/Usable/reshaped_draws.dta", clear
gen age_group_id = id+1
...
save "$j/temp/user/LRI/By Age/Pneumo/pcv_paf_byage.dta", replace
...
// Import Hib results for age_group_id 1 //
	use location_id year_id age draw* using "$j/temp/user/LRI/By Age/Hib/hib_paf_draws.dta", clear
	keep if age== "1"
	forval i = 1/1000 {
...
// No Hib PAF above 5 years //
			...
			tempfile main2
			save `main2', replace
			save "$j/temp/user/LRI/By Age/Pneumo/paf_`ag'_`date'.dta", replace
...
save "$j/temp/user/LRI/By Age/Pneumo/`pat'_paf_draws.dta", replace 
