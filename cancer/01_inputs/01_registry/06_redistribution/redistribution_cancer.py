...
def get_packages(input_data, package_folder, cause_map):
    ...
    if len(packages) == 0:
        input_data.to_stata(output_folder+'/post_rdp.dta', write_index=False)
        os._exit(0)
    return packages
...
def redistribute_garbage(data, proportions, package):
    ...
    # Merge with proportions with the full data set
...
input_data = '%s/split_%i/intermediate/for_rdp.dta' % (rdp_data_folder, SPLIT_GROUP)
output_folder = '%s/split_%i/final' % (rdp_data_folder, SPLIT_GROUP)
package_folder = '{}/{}'.format(MAP_FOLDER, PACKAGE_MAP)
cause_map = '{}/_package_map.csv'.format(package_folder)
signature_ids = ['global', 'dev_status', 'super_region', 'region', 'country', 'subnational_level1', 'subnational_level2', 'gbd_iteration', 'source', 'NID', 'registry', 'location_id', 'national', 'subdiv', 'year_start', 'year_end', 'sex', 'coding_system', 'split_group', 'age']
...
if MAGIC_TABLE == 1:
    ...
    output_data, diagnostics, signature_metadata, proportion_metadata = run_redistribution(input_data, signature_ids, proportion_ids, cause_map, package_folder, residual_cause=residual_cause)
print " --- %s seconds --- " % (time.time()-start_time)
output_data = output_data.ix[:,['location_id', 'gbd_iteration', 'source', 'NID', 'registry', 'national', 'subdiv', 'year_start', 'year_end', 'sex', 'coding_system', 'split_group', 'age', 'cause', 'freq']]
# 

## set datatypes and export to stata. multiply freq by a factor of 100 to reduce stata numerical formatting errros    
for c in ['location_id', 'national', 'year_start', 'year_end', 'NID', 'sex', 'gbd_iteration', 'split_group']:
    output_data[c] = output_data[c].fillna(-1).astype('int64')
for c in ['freq', 'age']:
    output_data[c] = output_data[c].astype('float64')
for c in ['registry', 'subdiv', 'source', 'coding_system', 'cause']:
