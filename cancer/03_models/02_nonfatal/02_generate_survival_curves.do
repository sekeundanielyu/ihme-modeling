...
// Accept or default arguments
	args use_2010_survival_curves

	if "`use_2010_survival_curves'" == "" local use_2010_survival_curves = 0
...
// Directory of extracted survival data from SEER
	local survival_input_dir "$j/WORK/07_registry/cancer/02_database/02_survival"
...
// Save again with the 1950- 2010 dataset
...
** ******************************************
// Use GBD 2010 survival curves for most causes, if requested
if `use_2010_survival_curves' == 1 {
	do "$code_folder/worker_scripts/02_use_old_survival_curves.do"
}
	
// Use gallbladder cancer from GBD 2010 
	preserve
		clear
		gen sex = .
		...
		foreach sex_letter in M F {
			insheet using "$j/Project/GBD/Systematic Reviews/ANALYSES/MALIGNANT NEOPLASMS/SEQUELAE/Copy of Gallbladder_`sex_letter'_cancer_survival_difference.csv", comma clear
			keep if year == 2010
