...
// save all data in memory
	tempfile tempDropReason
	save `tempDropReason', replace
	
// keep only the data that will be dropped
	drop if toDrop == 0
	capture confirm variable groupid
	if _rc != 0 keep iso3 source subdiv registry year year_span acause uid gbd_iteration national dropReason
	else keep iso3 source subdiv registry year year_span acause uid groupid gbd_iteration national dropReason
