...
// Description:	Submit jobs to estimate probability of long-term disability
...
// filepaths
	local inj_dir "$prefix/WORK/04_epi/01_database/02_data/_inj/04_models/gbd2015"
	local data_dir "`tmp_dir'/02_temp/03_data"
	local n_hs_map "`inj_dir'/02_inputs/parameters/maps/ncode_dws_map.csv"
	...
	local log_dir "`out_dir'/02_temp/02_logs"
	
	** ALTERING ADOPATH
	adopath + "`code_dir'/ado"
	run "`gbd_ado'/get_step_num.ado"
	...
	** getting locations of hierarchies and pooled followup-study data
	// Changed to J: location rather than clustertmp for GBD2015
	foreach i in hierarchies pooled_followup {
		get_step_num, name("`i'") stepfile("`code_dir'/_inj_steps.xlsx")
		local `i'_name ${step_num_`i'}_`i'
		local `i'_input_dir "WORK/04_epi/01_database/02_data/_inj/04_models/gbd2015/03_steps/`date'/``i'_name'/03_outputs/03_other"
...
// Get DW draws
	...
	local dw_file "`data_dir'/lt_t_dws_by_ncode.csv"
	
// Import hierarchies
	local h_loc "`hierarchies_input_dir'/hierarchies.xls"
...
// Import pooled followup studies
	import delimited using "`pooled_followup_input_dir'/pooled_followup_studies.csv", delim(",") clear case(preserve)
...
// Merge on hierarchy
	...
	forvalues x = 0/1 {
		use `both_pf', clear
		** currently attribute patients who had both inpatient and outpatient injuries to inpatient
		** TODO: Keep track of which injury is which back in the dataset cleaning/prepping 
		** code so that when applying the hierarchy we can select the inpatient injury 
		** b/c this is likely more severe
...
// Run regression and save draws of long-term probabilities by inpatient/outpatient/pooled
	** set holds macro and make directory for draws and check files
	...
	// submit job to run regression
		...
		!qsub -o /share/temp/sgeoutput/ngraetz/output -P proj_injuries -N `jobname' -pe multi_slot `slots' -l mem_free=`mem' "`code_dir'/stata_shell.sh" "`code_dir'/`step_name'/00_run_regression.do" "`inp' `tmp_dir' `data_dir' \"`no_lt'\" \"`all_lt'\" `data_dir' `code_dir' `dw_file' `gbd_ado' `diag_dir' `jobname' `slots'"
...
if $summary == 1 {
// submit job to get summary stats of untreated long-term probabilities for each platform
	local jobname `functional'_ss_by_pf
	!rm -r "`check_file_dir'"
	mkdir "`check_file_dir'"
	!qsub -P proj_injuries -N `jobname' "`code_dir'/stata_shell.sh" "`code_dir'/`step_name'/01_summary_stats_by_platform.do" "$prefix `data_dir' \"${S_ADO}\" `gbd_ado' `diag_dir' `jobname' `check_file_dir' `log_dir' `code_dir'"
	
	local count = 0
	while `count' == 0 {
		local checks : dir "`check_file_dir'" files "*.txt", respectcase
		local count : word count `checks'
		di "checking `c(current_time)': summary job not finished successfully"
		if `count' == 0 sleep 60000
	}
// End loop to summarize regression results
}	
...
// using summary stats, correct for instances in which we don't have follow-up data, or have small sample sizes, or strange results (i.e. inpatient < outpatient probabilities)
	import delimited using "`in_dir'/parameters/inp_only_ncodes.csv", delim(",") varnames(1) clear
	gen drop = 1
	tempfile dropns
	save `dropns', replace
	
	import delimited using "`data_dir'/WITH_AGE_long_term_prob_summary_stats.csv", delim(",") clear 
...
** add N10 and N36 if they don't exist at the draw level	(don't come as result of regressions- no data)
	...
	sort code
	** should make some sort of fail-safe to verify that all the ncodes are present (should just be missing 29)
	// isid code
	...
	** if we're replacing inpatient N8 with N9, then outpatient N8 is higher than inpatient N8...we'll make them the same for now
	cap if (`burn_indic' == 1) {
		forvalues i = 0/999 {
	...
	** verify N5 inpatient and outpatient set to 1- don't know why it isn't
	forvalues i = 0/999 {
		forvalues j = 0/2 {
	...
	** drop ncodes with zero long term probability
	...
	// EDIT 9/10/14 ng - Set spinal lesion lt probabilities to 98.7% manually.  This was derived from a single study on the long-term consequences of spinal injuries (Marino et al., 1999) because our regression method was giving implausibly low probabilities at all ages (~10%), which is likely a data problem.
		forvalues i = 0/999 {
			replace draw_`i' = rnormal(0.987, 0.004) if n_code == "N33" | n_code == "N34"
			replace draw_`i' = 1 if draw_`i' > 1 & (n_code == "N33" | n_code == "N34")
		}
	// EDIT 9/11/14 ng - Set medical complications to 0% manually.  We only have 20 cases from MEPS and they're all extremely high DWs, leaving us with 100% long-term probability under our current analysis.  The initial raw incidence data is also extremely high, especially in the US, so giving this a high lt prob is exploding the YLDs. We've decided we are forced to assume 0% until a better solution is created or better data is collected.
		forvalues i = 0/999 {
			replace draw_`i' = 0 if n_code == "N46"
		}
	// EDIT 9/30/14 ng - Set outpatient lt probabilities to 0 for contusions and poisonings.  These are high in the data but don't make sense to go long-term from outpatient.
		forvalues i = 0/999 {
			replace draw_`i' = 0 if (n_code == "N44" | n_code == "N41") & inpatient == 0 
		}
	export delimited using "`tmp_dir'/03_outputs/03_other/compiled_ltp_by_ncode_platform.csv", delim(",") replace
	
// Output draws
	local code = "prob_long_term/02_output_draws.do"
	local type = "epi"
	...
	** submit jobs
		...
		// Create check file upon submission and then delte when job is complete
			local name _`step_num'_`location_id'
			! qsub -P proj_injuries	-N `name' -pe multi_slot 4 -l mem_free=8 "`code_dir'/stata_shell.sh" "`code_dir'/`code'" "`root_j_dir' `root_tmp_dir' `date' `step_num' `step_name' `code_dir' `location_id' `year'"
			local n = `n' + 1
		}
