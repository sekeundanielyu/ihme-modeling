...
// Puropse: Prep the hospital data for creation of EN-matrices;
	...
	if `check'==1 {
		...
		local 3 "03b"
		local 4 "EN_matrices"
		local 5 "/share/code/injuries/ngraetz/inj/gbd2015"
		local 6 "/share/injuries/03_steps/2016_02_08/03b_EN_matrices/02_temp/03_data/00_cleaned"
		local 7 "/share/injuries/03_steps/2016_02_08/03b_EN_matrices/02_temp/03_data/01_prepped"
		local 8 "/snfs1/WORK/04_epi/01_database/02_data/_inj/04_models/gbd2015/02_inputs"
		local 9 "/snfs1/WORK/04_epi/01_database/02_data/_inj/04_models/gbd2015"
		local 10 "cleaned_ihme_data_deaths"
	}
	...
	// code repo
	local code_dir `5'
	// $prefix/WORK/04_epi/01_database/02_data/`functional'/04_models/`gbd'/03_steps/`date'/`step_num'_`step_name'/02_temp/03_data/00_cleaned"
	local cleaned_dir `6'
	// $prefix/WORK/04_epi/01_database/02_data/`functional'/04_models/`gbd'/03_steps/`date'/`step_num'_`step_name'/02_temp/03_data/01_prepped"	
	local prepped_dir `7'
	...
	** get the step number & filepath to the hierarchies
	get_step_num, name(hierarchies) stepfile("`code_dir'/_inj_steps.xlsx")
	local hierarchies_dir = "`root_j_dir'/03_steps/`date'/${step_num_hierarchies}_hierarchies"
	di "`hierarchies_dir'"
	...
	** this list contains all of the e-codes that are single models
	...
	import excel "`hierarchies_dir'/03_outputs/03_other/hierarchies.xls", sheet("noninp") firstrow clear
	...
	** bring in the cleaned hospital data
	insheet using "`cleaned_dir'/cleaned_`dataset'.csv", comma names clear
	...
	** now replace all non-inpatient-only n-codes with "" but only for those who had n-codes warrenting inpatient care but were marked as outpatient
	...
	foreach platform of local platforms {
		...
		** bring in hierarchy
		import excel "`hierarchies_dir'/03_outputs/03_other/hierarchies.xls", sheet("`platform'") firstrow clear
	...
	** end platform loop
	...
	outsheet using "`prepped_dir'/prepped_`dataset'.csv", comma replace
