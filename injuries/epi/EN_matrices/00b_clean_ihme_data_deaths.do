...
if "`1'"=="" {
	local 1 "/snfs1"
	local 2 "/share/code/injuries/ngraetz/inj"
	local 3 "/share/injuries/03_steps/2016_02_08/03b_EN_matrices/02_temp/03_data/00_cleaned"
	local 4 "`1'/WORK/04_epi/01_database/02_data/_inj/04_models/gbd2015/03_steps/2016_02_08/03b_EN_matrices/02_temp/02_logs"
	local 5 "`1'/WORK/10_gbd/00_library/functions"
}
...
foreach source of local hospital_dir_list {
	...
	** want to use only sources where the "01_mapped.dta" file exists
	...
	if !_rc {
		...
		** bring in the data; might be very large so try restricting what you are bringing in to just the variables you need to save some time
		cap use iso3 year age sex platform *yld_cause* metric_deaths using "`source_file'", clear
		if _rc != 0 {
	...
	** save appended/cleaned sources
	compress
	export delimited using "`output_data_dir'/cleaned_ihme_data_deaths.csv", delim(",") replace
