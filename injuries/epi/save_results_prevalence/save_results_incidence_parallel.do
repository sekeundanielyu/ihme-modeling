...
local inc_dir = "/share/injuries/04_COMO_input"
...
// short-term inpatient
	...
	save_results, modelable_entity_id(`me_id') description("Incidence for YLDs paper v1") in_dir("`inc_dir'/03_st_inc/inpatient/`ecode'") metrics(6) mark_best("yes")
...
// short-term outpatient
	use `master_me_ids', clear
		...
		keep if e_code == "`ecode'"
		local me_id = modelable_entity_id
	save_results, modelable_entity_id(`me_id') description("Incidence for YLDs paper v1") in_dir("`inc_dir'/03_st_inc/outpatient/`ecode'") metrics(6) mark_best("yes")
