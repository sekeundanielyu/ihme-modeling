...
setwd(paste0(root,"/WORK/04_epi/01_database/02_data/hiv/no_art/"))

#################################################################################
##BRING IN DATA AND FORMAT
#################################################################################
# LOGIT HAZARD, RANDOM EFFECT STUDY NO WEIGHTS NO SEX
df.re <- read.csv("./data/regression/output/logit_hazard_draws_re_study.csv")
...
pdf(file = "./graphs/model_draws/logit_hazard_draws_re_study.pdf", height = 6, width = 8)
...
# #################################################################################
# pdf(file = "./figures/model_draws/logit_hazard_draws_re_study_compare_to_fixed.pdf", height = 5, width = 5)
# 
# for(a in unique(df.re$age_cat)) {
#   
#   # # LOGIT HAZARD, RANDOM EFFECT STUDY NO WEIGHTS
