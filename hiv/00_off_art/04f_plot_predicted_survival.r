...
graph_dir <- paste0(root,"strPath")


#################################################################################
##BRING IN DATA AND FORMAT
#################################################################################
surv.preds <- read.csv(paste0(data_dir,"/compartmental_model/output/survival_curve_preds.csv"))
surv.preds <- subset(surv.preds, yr_since_sc <= 12)

# LOGIT HAZARD, RANDOM EFFECT STUDY NO WEIGHTS NO SEX
df.re <- read.csv(paste0(data_dir,"/regression/output/logit_hazard_draws_re_study.csv"))
...
pdf(file = paste0(graph_dir,"/model_draws/comp_model_pred_survival_draws_re_compare.pdf"), height = 5, width = 5)
