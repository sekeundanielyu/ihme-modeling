...
if (Sys.info()[1] == "Linux") {
  ...
  code_dir <- paste0("/ihme/code/mortality/",user,"/hiv/no_art")
} else {
  root <- "J:"
  user <- Sys.getenv("USERNAME")
  code_dir <- paste0("C:/Users/",user,"/Documents/Git/hiv/no_art")
}
...
data_dir <- paste0(root,"/strPath")
extract_dir <- paste0(root,"/strPath")
spectrum_dir <- paste0(root,"/strPath")
draw_save_dir <- "/strPath"
...
qsub <- function(jobname, code, hold=NULL, pass=NULL, slots=1, submit=F) { 
  ...
  sub <- paste("/share/local/UGE8.3.1p6-prod/bin/lx-amd64/qsub", 
...
if (archive == T) {
  ...
  time_counter <- 0
  while(counter == 0) {
    test <- file.exists(paste0("median_survival/matched_draw_median_survival_stats.csv"))
    if(test == T) {
      counter <- 1
  ...
  if (start<=5) {
    for(fff in c("re_age_specific_parameter_bounds","coeff_var_table")) {
      file.copy(paste0("compartmental_model/output/",fff,".csv"),
                paste0("compartmental_model/output/archive/",fff,"_",run_date,".csv"),
                overwrite = T) 
    }
