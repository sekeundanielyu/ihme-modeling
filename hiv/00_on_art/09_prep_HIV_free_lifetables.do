...
// locals
	...
	local extract_file "strPath/HIV_extract_KM_2015.xlsx"
	local store_data "strPath/hiv_free_life_tables"
...
** *******************************
	...
	// Keep vars and age groups of interest... 
		drop if age==0 | age==1 | age==105 | age==110 |  age==5 | age==10
	...
	// Prep population
	// Note: Most countries should have granular over-80 pops
	// But double check it before 
		use ihme_loc_id year sex_id age_group_id age_group_name pop using "`GBD_pop'" if sex_id != 3 & age_group_id <=20 & age_group_id >= 8 | inlist(age_group_id,30,31,32,33,48) , clear // Keep 15-100 populations and sex-specific
		di in red "These countries are missing the more granular pops"
		levelsof ihme_loc_id if age_group_id == 33 & pop == . & year == 2010
