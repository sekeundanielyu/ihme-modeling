...
local HR_data "strPath/HIV_extract_HR_2015.xlsx"
local dismod_dir "strPath"
local pct_dir "strPath" 
...
************ PREP DATA **********
	...
	** In excel, already standardized everything to be referenced to the lowest age category
	...
	replace super="other" if gbd_region=="Latin America/Carribean" | gbd_region=="Central Asia" | gbd_region=="North Africa and Middle East" | gbd_region=="South Asia" | gbd_region=="Southeast Asia"   
...
foreach sup in ssa high {
	...
	// Set up data_in sheet
		// Want a random effect depending on the study, since they all have different reference categories
	...
	// Update the 'draw_in' file (from hazard ratio dismod_ode file)
		insheet using $root/WORK/04_epi/01_database/02_data/hiv/on_art/data/pct_male_med_age/pct_male_med_age.csv, comma names clear
		...
		replace age_lower=`median_age' if row_name=="agehr_median"
		replace age_upper=`median_age' if row_name=="agehr_median"
