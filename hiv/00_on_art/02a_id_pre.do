...
local dismod_templates "$root/WORK/04_epi/01_database/02_data/hiv/dismod_templates"
local bradmod_dir "$root/WORK/04_epi/01_database/02_data/hiv/on_art/data/bradmod"
local store_best_dir "$root/WORK/04_epi/01_database/02_data/hiv/on_art/data/id_best"
local graph_dir "$root/WORK/04_epi/01_database/02_data/hiv/on_art/graphs/Manuscript"
local dismod_dir "$root/temp/dismod_ode"
...
******** CONDITIONAL **********
	...
	***************** SET UP FOR BRAD MOD  - CONDITIONAL *****************
	...
	// Save plain_in, and effect_in value_in and rate_in csvs
		...
		foreach dur in 0_6 7_12 12_24 {
			insheet using "$root/WORK/04_epi/01_database/02_data/hiv/dismod_templates/plain_in.csv", clear comma names	
			outsheet using "`dismod_dir'/HIV_IDBEST_`dur'//plain_in.csv", comma names replace
		}
		
		foreach dur in 0_6 7_12 12_24 {
			insheet using "$root/WORK/04_epi/01_database/02_data/hiv/dismod_templates/value_in.csv", clear comma names
			outsheet using "`dismod_dir'/HIV_IDBEST_`dur'//value_in.csv", comma names replace
		}
		
		
		foreach dur in 0_6 7_12 12_24 {
			insheet using "$root/WORK/04_epi/01_database/02_data/hiv/dismod_templates/effect_in.csv", clear comma names
			drop if inlist(name, "high", "ssa", "other")
		...
		foreach dur in 0_6 7_12 12_24 {
		insheet using "$root/WORK/04_epi/01_database/02_data/hiv/dismod_templates/rate_in.csv", clear comma names
			drop if type=="diota" & age==50
			drop if type=="iota" & age==100
			replace age=50 if type=="chi" & age==100
			replace age=50 if type=="omega" & age==100
