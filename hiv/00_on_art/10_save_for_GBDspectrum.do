...
// Last Updated by: Emily Dansereau
...
/// locals
	...
	local data_dir "$root/WORK/04_epi/01_database/02_data/hiv/on_art/data"
	local store_draws "$root/WORK/04_epi/01_database/02_data/hiv/on_art/data/hiv_on_art_draws"
	// local save_spectrum "$root/Project/Causes of Death/codem/models/A02/GBD 2013 HIV/Program_inputs/data/AIM_assumptions/transition_parameters/HIVmort_onART_regions/Mar27"
	local stage_dir = "$root/WORK/04_epi/01_database/02_data/hiv/on_art/data/probs_by_region"
	local save_spectrum "$root/WORK/04_epi/01_database/02_data/hiv/04_models/gbd2015/02_inputs/AIM_assumptions/transition_parameters/HIVmort_onART_regions"
	local spectrum_archive_dir "/ihme/gbd/WORK/04_epi/01_database/02_data/hiv/04_models/gbd2015/02_inputs/AIM_assumptions/transition_parameters/HIVmort_onART_regions" 
	...
	local store_graphs "$root/WORK/04_epi/01_database/02_data/hiv/on_art/graphs/hiv_km"
	local spec_xbs "`save_spectrum'/UNAIDS_HIVmort_onART.csv"
	local HIV_free_mort "$root/WORK/04_epi/01_database/02_data/hiv/on_art/data/hiv_free_life_tables/HIV_free_spectrum_dth_rts.csv"
...
** ****************************
	...
	save "$root/WORK/04_epi/01_database/02_data/hiv/on_art/data/mortality_probs.dta", replace
	...
	if (c(os)=="Unix") {
		...
		// Launch filesaving jobs
			...
			foreach region in `regions' {
				...
				! /usr/local/bin/SGE/bin/lx24-amd64/qsub -e /share/temp/sgeoutput/$user/errors -o /share/temp/sgeoutput/$user/output -P proj_hiv -pe multi_slot 2 -N save_`region' "run_all.sh" "$code_dir/10b_save_parallel.do" "`region' `super_submit' $archive $date"
			}
		*/
...
/// GRAPH TO CHECK SOUTH AFRICA VERSUS SEPCTRUM ASSUMPTIONS
	...
	insheet using "`save_spectrum'/default_EPP_combo/ZAF_HIVonART.csv", clear comma names
	gen source="spectrum"
	tempfile spectrum
	save `spectrum', replace
	
	insheet using "`save_spectrum'/DisMod/ZAF_HIVonART.csv", clear comma names
	gen source="IHME"
	...
	pdfstart using "$root/WORK/04_epi/01_database/02_data/hiv/on_art/graphs/death_prob_`dur'_annual.pdf"
...
** ************************************
	...
	// Append together and graph... 
		...
		** keep regions of interest... 
		...
		// Graph durations
			pdfstart using "`store_graphs'/Death_rts_IHME_v_UNAIDS_durations.pdf"
				** 0-6
				use "`tmp_data'", clear
					keep if dur=="LT6Mo"
					levelsof sex, local(sexes)
...
/* 		global color1 "255 255 191"
		...
		global color7 "69 117 180"
			...
			pdfstart using "$root/WORK/04_epi/01_database/02_data/hiv/on_art/graphs/Manuscript/cumulative mortality SSA 25-34.pdf"
		...
		// AFRICA
		pdfstart using "`store_graphs'/Cumulative_Deaths_IHME_v_UNAIDS_SSA.pdf"
		...
		// HIGH
		pdfstart using "`store_graphs'/Cumulative_Deaths_IHME_v_UNAIDS_HIGH.pdf"
		...
		// OTHER
		pdfstart using "`store_graphs'/Cumulative_Deaths_IHME_v_UNAIDS_OTHER.pdf"
			foreach age in 15-25 25-35 35-45 {
				foreach sex in Male Female {
					foreach cd4_cat in ARTLT50CD4 ART50to99CD4 ART100to199CD4 ART200to249CD4 ART250to349CD4 ART350to500CD4 ARTGT500CD4 {
						use `pre', clear
