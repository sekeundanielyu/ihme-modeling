...
prior <- function(theta){
  return(unlist(mclapply(seq_len(nrow(theta)), function(i) return(exp(lprior(theta[i,], fp))), mc.cores=5)))
}

likelihood <- function(theta){
  return(unlist(mclapply(seq_len(nrow(theta)), function(i) return(exp(ll(theta[i,], fp, likdat))), mc.cores=5)))
}
