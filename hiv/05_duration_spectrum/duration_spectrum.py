...
# python_shell = '/home/j/Project/Causes of Death/codem/models/A02/GBD 2013 HIV/Estimates/code/cluster_shell.sh'
...
def lambdaCoeff(age, state):
	'''Get progression probabilities.
	(Not needed with GBD 2013 progression estimates)'''
	if state == 'LT50CD4':
		return 0.0000001
	else:
		if age > 45:
			age10 = len(averageCD4Duration)-1
		else:
			age10 = ((age - (age - 5) % 10) - 15) / 10
		meanDuration = averageCD4Duration[age10][state]
		return (1 - math.e**(-1 * 1/meanDuration/timeStep))
...
if locations['survivalRates'] != 'gbd_draws':
	inputSRdata = beers.openCSV(demProjFilePath + locations['survivalRates'] + "/life_tables/" + loc_id + "_life_tables.csv")
else:
	# tmp_loc = ISO
	# if 'IND' in regionCode:
	# 	tmp_loc = loc_id
	inputSRdata = beers.openCSV('/snfs3/WORK/02_mortality/03_models/5_lifetables/results/hivfree_sx/locs/' + loc_id + "_life_tables.csv")

yearIndexSurv = inputSRdata[0].index('year')
sexIndexSurv = inputSRdata[0].index('sex')
ageIndexSurv = inputSRdata[0].index('age')
# exIndexSurv = inputSRdata[0].index('ex')
if locations['survivalRates'] != 'gbd_means':
...
if 'paired' in locations['noARTmortality']:
	inputNoARTMortality = beers.openCSV(AIMfilePath + "strPath/" + locations['noARTmortality'] + '/' + regionCode + "_mortality_par_draws.csv")
else:
	inputNoARTMortality = beers.openCSV(AIMfilePath + "strPath/" + locations['noARTmortality'] + '/' + ISO + "_mortality_par_draws.csv")

if locations['onARTmortality'] == 'default_EPP_combo':
	inputOnARTMortality = beers.openCSV(AIMfilePath + "strPath/" + locations['onARTmortality'] + "/" + ISO + "_HIVonART.csv")
elif 'paired' in locations['onARTmortality']:
	inputOnARTMortality = beers.openCSV(AIMfilePath + "strPath/" + locations['onARTmortality'] + "/" + regionCode + "_HIVonART.csv")
else:
	inputOnARTMortality = beers.openCSV(AIMfilePath + "strPath/" + locations['onARTmortality'] + "/" + ISO + "_HIVonART.csv")
...
if 'paired' in locations['averageCD4duration']:
	inputProgressionParameters = beers.openCSV(AIMfilePath + "strPath/" + locations['averageCD4duration'] + "/" + regionCode + "_progression_par_draws.csv")
else:
	inputProgressionParameters = beers.openCSV(AIMfilePath + "strPath/" + locations['averageCD4duration'] + "/" + ISO + "_progression_par_draws.csv")

inputAdultARTeligibility = beers.openCSV(AIMfilePath + "strPath/" + locations['adultARTeligibility'] + '/' + new_loc + "_adult_eligible.csv")

if 'averages' in locations['percentBF']:
	inputTFRreduction = beers.openCSV(AIMfilePath + "strPath/" + locations['TFRreduction'] + "/" + regionCode + "_TFR_ratio.csv")
else:
	inputTFRreduction = beers.openCSV(AIMfilePath + "strPath/" + locations['TFRreduction'] + "/" + ISO + "_TFR_ratio.csv")

inputAdultARTCoverage = beers.openCSV(AIMfilePath + "strPath/" + locations['adultARTcoverage'] + '/' + new_loc + "_Adult_ART_cov.csv")
...
# if 'forecast' in locations['adultARTcoverage']:
# 	iso3Index = inputAdultARTCoverage[0].index('iso3')
# 	for row in inputAdultARTCoverage[1:]:
# 		row[iso3Index] = 0
...
inputPMTCT = beers.openCSV(AIMfilePath + 'strPath/' + locations['PMTCT'] + '/' + new_loc + '_PMTCT_ART_cov.csv')
yearIndexPMTCT = inputPMTCT[0].index('year')
# if 'forecast' in locations['PMTCT']:
# 	for v in ['iso3', 'location_name', 'super_region_name', 'region_name']:
# 		tmpIndex = inputPMTCT[0].index(v)
# 		for row in inputPMTCT[1:]:
# 			row[tmpIndex] = 0
...
try:
	inputPostnatalDropout = beers.openCSV(AIMfilePath + '/strPath/' + locations['PMTCTdropoutRates'] + '/' + new_loc + '_PMTCT_mth_drop_rts.csv')
except:
	inputPostnatalDropout = beers.openCSV(AIMfilePath + '/strPath/' + locations['PMTCTdropoutRates'] + '/' + "DEF" + '_PMTCT_mth_drop_rts.csv')
		
if locations['percentBF'] == 'defaults':
	inputPercentBF = beers.openCSV(AIMfilePath + '/strPath/' + locations['percentBF'] + '/' + new_loc + '_notBF_pct.csv')
elif 'averages' in locations['percentBF']:
	inputPercentBF = beers.openCSV(AIMfilePath + '/strPath/' + locations['percentBF'] + '/' + regionCode + '_notBF_pct.csv')
	inputPercentBF[0] = [val.lower() for val in inputPercentBF[0]]
else:
	inputPercentBF = beers.openCSV(AIMfilePath + '/strPath/' + locations['percentBF'] + '/' + ISO + '_notBF_pct.csv')
	inputPercentBF[0] = [val.lower() for val in inputPercentBF[0]]

inputIncAgeDist = beers.openCSV(AIMfilePath + "/strPath/GEN_IRR.csv")

# Get incidence and prevalence
inputEPPinc = beers.openCSV(demProjFilePath + "strPath/" + locations['incidence'] + '/' + regionCode + "_SPU_inc_draws.csv")
inputEPPprev = beers.openCSV(demProjFilePath + "strPath/" + locations['prevalence'] + '/' + regionCode + "_SPU_prev_draws.csv")
...
inputInitCD4dist = beers.openCSV(AIMfilePath + "strPath/" + locations['initCD4dist'] + "/CD4count_distribution_at_infection.csv")

inputMTCtransRates= beers.openCSV(AIMfilePath + "strPath/" + locations['MTCtransRates'] + "/PMTCT_transmission_rts.csv")

inputChildCD4countDist = beers.openCSV(AIMfilePath + 'strPath/' + locations['childCD4countDist'] + '/childCD4countDist.csv')

inputChildCD4percentDist = beers.openCSV(AIMfilePath + 'strPath/' + locations['childCD4percentDist'] + '/childCD4percentDist.csv')

inputChildARTelgibility = beers.openCSV(AIMfilePath + "strPath/" + locations['childARTeligibility'] + '/' + new_loc + "_child_eligible.csv")

inputChildARTCoverage = beers.openCSV(AIMfilePath + 'strPath/' + locations['childARTcoverage'] + '/' + new_loc + '_Child_ART_cov.csv')

inputChildHIVmortality = beers.openCSV(AIMfilePath + "strPath/" + locations['childHIVmortality'] + "/HIVtoAIDSdeath_children.csv")

inputChildCTXeffect = beers.openCSV(AIMfilePath + "strPath/" + locations['childCTXeffect'] + "/Child_treatment_effects_cotrim.csv")

inputEligiblePops = beers.openCSV(AIMfilePath + "strPath/" + locations['eligiblePops'] + "/pops_eligible_regardless_cd4.csv")

# regionData = beers.openCSV(AIMfilePath + "classification/region_class/" + locations['region'] + "/region_class.csv")
# isoIndex = regionData[0].index('iso3')
# regionIndex = regionData[0].index('region')
# region = [row for row in regionData if row[isoIndex] == ISO][0][regionIndex]
...
for run_id in xrange(indiv_run_nums):
	...
	np.random.seed(individual_id)
	random.seed(individual_id)
	adjustments = {}
		...
		try:
			...
			for p in treatTypes:
				...
				if not ARTcounterFactual:
					tmp_num = np.array([float(row[col_nums[0]]) for row in inputPMTCT[1:]])
					tmp_pct = np.array([float(row[col_nums[1]]) for row in inputPMTCT[1:]])
				else:
			...
			var_names = ['year', 'age', 'sex']
			pop_vars = ['p_' + str(i) for i in xrange(minYear, maxYear+1)]
			death_vars = ['d_' + str(i) for i in xrange(minYear, maxYear+1)]
			var_names.extend(pop_vars)
			var_names.extend(death_vars)
			csvData.append(var_names)
			t=minYear
			for a in xrange(15, maxAge+1):
				for s in sexes:
					tmp_row = [t, a, s]
					out_pop = [0.0 for i in xrange(minYear, maxYear+1)]
					out_deaths = [0.0 for i in xrange(minYear, maxYear+1)]
					tmp_row.extend(out_pop)
					tmp_row.extend(out_deaths)
					csvData.append(tmp_row)
			...
			ART_vars = ['year', 'age', 'sex', 'type', 'pop_covered', 'pop_eligible']
			ART_CSV = []
			ART_CSV.append(ART_vars)
			for a in ['adult', 'child']:
				for s in sexes:
					for c in allInterventions:
						tmp_row = [minYear, a, s, c, 0.0, 0.0]
						ART_CSV.append(tmp_row)
			...
			for t in xrange(minYear + 1, maxYear + 1):
					...
					for sex in sexes:
						...
						for age in xrange(15, 81):
								for c in noARTCD4states:
									...
									if sex =='female' and age in xrange(15, 50):
										eligibleAdults[t - minYear][sex] += getEligiblePregnantWomen(age, t)
						# if t1 in [1,2] and t == 2002:
						# 	print 'Eligible Adults %s t%i:\t%f' % (sex, t1, eligibleAdults[t-minYear][sex])
						...
						prop2 = {}
						# if t1 in [1, 2] and t == 2002:
						# 	print 'new art %s t%i:\t%f' % (sex, t1, newART)
						# 	print 'needed art %s t%i:\t%f' % (sex, t1, neededART)
						# 	print 'ART survivors %s t%i:\t%f' % (sex, t1, ARTsurvivors)
						# 	# print 'art survivors > 25 %s t%i:\t%f' % (sex, t1, test_aggregate[50])
						# 	# print 'art survivors <= 25 %s t%i:\t%f' % (sex, t1, test_aggregate[25])
						...
						sum3 = 0
						# if t1 == 2:
						# 	print 'Eligible Adults LT50 %s:\t%f' % (sex, eligibleAdultsCD4['LT50CD4'][sex])
						...
						# if t1 == 2 and t in [2001, 2002]:
						# 	print 'new patients LT50 %s:\t%f' % (sex, newPatients['LT50CD4'][sex])
						# 	print 'mortRate LT 50 %s:\t%f' % (sex, mortRate['LT50CD4'])
				...
				for age5 in xrange(15, 49, 5):
					...
					birthsToPosMothers['female'][age5] = (HIVwomen * TFR[t] * TFRreduction[age5c] / (TFRreduction[age5c] * prev2 + (1 - prev2))
						* (ASFRbyAge[age5][t-minYear] / np.sum(ASFRbyYear[t])) / 5)
					if age5 < 25:
						tempBirths15to24 += (HIVwomen * TFR[t] * TFRreduction[age5c] / (TFRreduction[age5c] * prev1 + (1 - prev1))
							* (ASFRbyAge[age5][t-minYear] / np.sum(ASFRbyYear[t])) / 5)
				print 'HIV Women:\t%f' % HIVwomen
				abortion = 0
				tempBirths -= abortion
				if tempBirths < 0:
					tempBirths = 0
				...
				print 'male need:\t%f' % needForFLARTmale
				print 'female need:\t%f' % needForFLARTfemale
				print 'total need:\t%f' % needForFLART
				...
				child_art = 0
				for a in xrange(0, childMaxAge+1):
					for s in sexes:
						for d in xrange(minDuration, maxChildDuration+1):
							child_art += sum(population[a][t][s]['onFLART'][d])
				tmp_hiv_women = 0
				for a in xrange(15, maxAge+1):
					for c in allCD4states:
						if c in noARTCD4states:
							tmp_hiv_women += sum(population[a][t]['female'][c])
						else:
							for d in adultARTdurations:
								tmp_hiv_women += sum(population[a][t]['female'][c][d])
				# print 'Child ART:\t%f' % child_art
				print 'HIV Women end year:\t%f' % tmp_hiv_women
				...
				for a in xrange(15, maxAge+1):
					for s in sexes:
						...
						for i in xrange(minYear, maxYear+1):
							...
							for c in allCD4states:
								if c in noARTCD4states:
									total_pop += population[a][t][s][c][i-minYear]
								else:
									for d in adultARTdurations:
										total_pop += population[a][t][s][c][d][i-minYear]
							tmp_pop.append(total_pop)
						tmp_row.extend(tmp_pop)
						tmp_deaths = []
						for i in xrange(minYear, maxYear+1):
				...
				for a in ['adult', 'child']:
					for sex in sexes:
						for c in allInterventions:
							tmp_row = [t, a, sex, c, allInterventionCoverage[t][a][sex][c]['coverage'], allInterventionCoverage[t][a][sex][c]['eligible']]
							ART_CSV.append(tmp_row)

			if writeFiles:
				results_dir = os.path.split(directory)[-1]
				# result_path = '/share/gbd/WORK/04_epi/01_database/02_data/hiv/01_lit/Spectrum_output/' + results_dir +'/'
				...
				if not os.path.isdir(writePath):
					try:
						...
						pass
				if counterfactual_type == 'no_ART':
					file_name = regionCode + '_no_ART_deaths_' + str(individual_id) + '.csv'
				elif counterfactual_type == 'ART':
					file_name = regionCode + '_ART_deaths_' + str(individual_id) + '.csv'
				elif counterfactual_type == 'HQ':
					file_name = regionCode + '_HQ_deaths_' + str(individual_id) + '.csv'
				elif counterfactual_type == 'HI':
					file_name = regionCode + '_HI_deaths_' + str(individual_id) + '.csv'
				...
				ART_filename = regionCode + '_coverage_' + str(individual_id) + '.csv'
				ARTfile = open(writePath + ART_filename, 'w')
				wr = csv.writer(ARTfile)
				for row in ART_CSV:
					wr.writerow(row)
				ARTfile.close()
