...
// WRITE CODE HERE
	...
	get_draws, gbd_id_field(modelable_entity_id) gbd_id(1419) measure_ids(7 9) location_ids(`location') source(epi) status(best) clear
	...
	foreach year of local years {
		foreach sex of local sexes {
			...
			drop measure_id
			merge 1:1 age_group_id using ``location'_`year'_`sex'', nogen
			...
			forvalues c = 0/999 {
				qui replace draw_`c' = exp(-draw_`c' * dur_`c')
